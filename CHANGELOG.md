...
- 验证方式（**不涉及本项目构建**，未跑 Gradle / NDK / CI）：① ktlint 1.8.0（本机，比 CI 用的 1.5.0 更新）对改动文件零违规——运行前先用探针文件确认该工具确实会报错，避免"零输出"被误读为"零违规"；全量 `app/src/**/*.kt` 扫描只剩**一处既有**违规（`McpHttpServer.kt` 类体内一段游离 KDoc，`standard:kdoc`），不在本次改动范围内、且 CI 版本 1.5.0 不报此项（依据：main 顶点 `ab6b1a7` 上 CI 步骤 `ktlint — Kotlin code style check` 结论为 success）。② 新增 4 个单元测试：桥接结果命中自身包（含从 `content[].text` 二次解析、并断言 `workspaceId` 被收进隔离清单）、第三方包不被误判、隔离后跟随调用在参数扫描阶段即被拒（`workspaceId` 与内嵌 `apk:<id>!lib/...` 两种形态）、`clearQuarantine()` 能复位；③ 本次未触及 native 与 JNI（无 `external fun` 变动），行宽按字符数复核（最长 148）。
