...
**标题：** 侵权投诉：帖子《`<帖子标题>`》违反 AGPL-3.0 开源协议、侵犯软件著作权

**正文：**

管理员您好，我是开源软件 **SOMCP**（https://github.com/bilieebiliee1-design/SOMCP ，AGPL-3.0-only）的著作权人。

现举报本站帖子：`<帖子链接>`，发布者：`<ID>`。

该帖发布的「`<侵权作品名>`」系基于我的 SOMCP 二次开发的衍生作品（核心功能、专有工具命名 `apk_analyze`/`edit_asm`/`build_so`/`flutter_blutter` 等、技术架构高度一致），但在分发时：未提供完整对应源代码、删除了原作者署名、未以 AGPL-3.0 继续授权、并以"原创"名义发布。此举违反 AGPL-3.0 第 4、5 条，依第 8 条其授权已自动终止，构成著作权侵权。
...
我是 SOMCP（https://github.com/bilieebiliee1-design/SOMCP ，AGPL-3.0-only）著作权人。贵平台下述分享链接承载的文件侵犯我的软件著作权并违反 AGPL-3.0：

- 分享链接：`<链接>`
- 文件名：`<文件名>`
- 侵权说明：该文件为我的开源软件的二改闭源分发版，未提供源代码、删除署名、未沿用 AGPL-3.0。
...
copyright/attribution removed, and not licensed under AGPL-3.0 — in breach of
AGPL-3.0 sections 4–5, terminating the license under section 8.
