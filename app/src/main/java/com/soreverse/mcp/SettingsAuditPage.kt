...
internal fun SettingsAuditPage(t: UiText, settings: SettingsStore) {
    ...
    var toolCallRateLimitPerMin by remember {
    ...
    PageScroll {
        GlassGroup {
            ToggleRow(
                if (t.zh) "编辑前自动快照" else "Auto-snapshot before edit",
                autoSnapshotBeforeEdit
            ) {
                autoSnapshotBeforeEdit =
                    it
                settings.autoSnapshotBeforeEdit = it
            }
            GroupDivider()
            ToggleRow(if (t.zh) "启用审计日志" else "Enable audit log", auditLogEnabled) {
                auditLogEnabled =
                    it
                settings.auditLogEnabled = it
            }
            GroupDivider()
            ToggleRow(if (t.zh) "审计持久化到磁盘" else "Persist audit to disk", auditPersist) {
                auditPersist =
                    it
                settings.auditPersist = it
            }
            GroupDivider()
            ToggleRow(if (t.zh) "收集工具调用统计" else "Collect tool stats", collectToolStats) {
                collectToolStats =
                    it
                settings.collectToolStats = it
            }
            GroupDivider()
            ...
            GroupDivider()
            ToggleRow(
                if (t.zh) "启用模拟执行 (Unidbg)" else "Enable emulation (Unidbg)",
                emulationEnabled
            ) {
                emulationEnabled =
                    it
                settings.emulationEnabled = it
            }
            Text(
        ...
        GlassGroup {
            NumberSettingRow(if (t.zh) "重型工具并发上限" else "Heavy tool concurrency cap", maxConcurrentTools, {
                maxConcurrentTools =
                    it
            }, { settings.maxConcurrentTools = it }, if (t.zh) "个" else "tasks")
            GroupDivider()
            NumberSettingRow(if (t.zh) "最大快照数" else "Max snapshots", maxSnapshots, {
                maxSnapshots =
                    it
            }, { settings.maxSnapshots = it }, if (t.zh) "个" else "items")
            GroupDivider()
            NumberSettingRow(if (t.zh) "最大审计文件数" else "Max audit files", maxAudits, {
                maxAudits = it
            }, {
                settings.maxAudits =
                    it
            }, if (t.zh) "个" else "files")
            GroupDivider()
            NumberSettingRow(if (t.zh) "diff 最大范围数" else "Max diff ranges", maxCompareRanges, {
                maxCompareRanges =
                    it
            }, { settings.maxCompareRanges = it }, if (t.zh) "段" else "ranges")
            GroupDivider()
            NumberSettingRow(if (t.zh) "日志显示上限" else "Max log lines", logMaxLines, {
                logMaxLines =
                    it
            }, { settings.logMaxLines = it }, if (t.zh) "行" else "lines")
            GroupDivider()
            NumberSettingRow(if (t.zh) "请求超时" else "Request timeout", requestTimeoutMs, {
                requestTimeoutMs =
                    it
            }, { settings.requestTimeoutMs = it }, "ms")
            GroupDivider()
            NumberSettingRow(if (t.zh) "工具结果上限" else "Tool result limit", toolResultMaxChars, {
                toolResultMaxChars =
                    it
            }, { settings.toolResultMaxChars = it }, if (t.zh) "字符" else "chars")
            GroupDivider()
            NumberSettingRow(if (t.zh) "工具调用频率限制" else "Tool call rate limit", toolCallRateLimitPerMin, {
                toolCallRateLimitPerMin =
                    it
            }, { settings.toolCallRateLimitPerMin = it }, if (t.zh) "次/分" else "/min")
        }
        GlassGroup {
            Text(
                if (t.zh) "禁用工具" else "Disabled tools",
                modifier = Modifier.padding(horizontal = 14.dp, vertical = 8.dp),
            ...
            ExposedDropdownMenuBox(
                ...
                OutlinedTextField(
                    ...
                    onValueChange = {
                        disabledTools = it
                        settings.disabledTools = it
                    },
                    label = {
                        Text(if (t.zh) "禁用工具(逗号分隔)" else "Disabled tools (comma-separated)")
                    },
                    placeholder = {
                        Text(if (t.zh) "从下拉列表选择或直接输入" else "Select from dropdown or type directly")
                    },
                    singleLine = true,
                    trailingIcon = {
                ...
                ExposedDropdownMenu(
                    ...
                    val disabledSet = disabledTools.split(
                        ','
                    ).map(String::trim).filter(String::isNotBlank).toMutableSet()
                    ToolCatalog.names.forEach { name ->
                        ...
                        DropdownMenuItem(
                            ...
                            onClick = {
                                val next = disabledSet.toMutableSet().apply {
