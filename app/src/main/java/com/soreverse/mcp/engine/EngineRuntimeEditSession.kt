...
internal fun EngineRuntime.editOpen(workspaceId: String): JSONObject = guarded {
    ...
    ok(
        JSONObject().put(
            "editSessionId",
            id
        ).put("workspaceId", workspaceId).put("initialTargetVersion", sha256(ws.data))
    )
}
    ...
    withSession(
        ...
        val snap =
            Snapshot(
                ...
                session.patches.size,
                session.data.copyOf(),
                protected = label.startsWith("batch-transaction-")
            )
    ...
    withSession(
        ...
        if (session.snapshots.isEmpty()) {
            return@withSession err(
                "NO_SNAPSHOT",
                "No snapshot exists in this session. Call session_history (action=snapshot) first."
            )
        }
        val idx = if (snapshotIndex < 0) session.snapshots.size - 1 else snapshotIndex
        if (idx !in
            session.snapshots.indices
        ) {
            return@withSession err(
                "SNAPSHOT_NOT_FOUND",
                "Snapshot index $idx out of range [0, ${session.snapshots.size - 1}]",
                "snapshotIndex",
                snapshotIndex
            )
        }
...
internal fun EngineRuntime.editUndo(workspaceId: String, editSessionId: String, count: Int = 1): JSONObject = guarded {
    withSession(
        ...
        if (session.patches.isEmpty()) {
            return@withSession err(
                "NOTHING_TO_UNDO",
                "No patches to undo in this session"
            )
        }
        ...
        repeat(n) {
            ...
            val oldBytes = p.oldHex.split(' ').filter {
                it.isNotBlank()
            }.map { it.toInt(16).toByte() }.toByteArray()
            if (p.fileOffset in
                0..(session.data.size - oldBytes.size)
            ) {
...
internal fun EngineRuntime.editRedo(workspaceId: String, editSessionId: String, count: Int = 1): JSONObject = guarded {
    withSession(
        ...
        if (session.undone.isEmpty()) {
            return@withSession err(
                "NOTHING_TO_REDO",
                "No undone patches to redo in this session"
            )
        }
        ...
        repeat(n) {
            ...
            val newBytes = p.newHex.split(' ').filter {
                it.isNotBlank()
            }.map { it.toInt(16).toByte() }.toByteArray()
            if (newBytes.isNotEmpty() &&
                p.fileOffset in 0..(session.data.size - newBytes.size)
            ) {
...
internal fun EngineRuntime.maybeAutoSnapshot(session: EditSession, trigger: String, settings: SettingsStore) {
    ...
    AppLog.i(
        "Auto-snapshot (trigger=$trigger) rev=${session.revision} patches=${session.patches.size}"
    )
}
...
internal fun EngineRuntime.maybeAutoPersist(workspaceId: String, session: EditSession, settings: SettingsStore): JSONObject? {
    ...
    return runCatching {
        val dir = auditDir()
        val existing =
            dir.listFiles { f -> f.isFile && f.name.startsWith("${session.id}-") }?.toList()
                ?: emptyList()
        if (existing.size >=
            settings.maxAudits
        ) {
