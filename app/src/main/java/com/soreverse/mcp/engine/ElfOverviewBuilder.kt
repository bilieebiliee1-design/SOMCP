...
object ElfOverviewBuilder {
    ...
    fun build(
        ...
        val exported = elf.dynSymbols.filter {
            it.exported ||
                (!it.imported && it.value > 0 && it.bind in setOf("GLOBAL", "WEAK"))
        }
        ...
        val segmentBasis = if (loadable.isNotEmpty()) {
            loadable
        } else {
            segmentRows.filter {
                it.filesz >
        ...
        val classSizes = segmentBasis.groupBy {
            classifySegment(it)
        }.mapValues { (_, rows) -> rows.sumOf { it.filesz } }
        ...
        val stackExec = elf.programHeaders.any {
            it.type == PT_GNU_STACK && it.flags and PF_X != 0L
        }
        val nxEnabled = if (hasGnuStack) !stackExec else true
        val flags = dynFirst(elf, DT_FLAGS)
        val flags1 = dynFirst(elf, DT_FLAGS_1)
        val bindNow =
            (flags and DF_BIND_NOW != 0L) || (flags1 and DF_1_NOW != 0L) || dynHas(elf, DT_BIND_NOW)
        ...
        val hasCfi = allSymbols.any {
            it.name.contains("__cfi_check") ||
                it.name.contains("__cfi_slowpath")
        }
        val hasAntiDebugSym = allSymbols.any {
            nameMatches(
                it.name,
                Regex(
                    "ptrace|inotify_add_watch|__android_log_is_debuggable",
                    RegexOption.IGNORE_CASE
                )
            )
        }
        val hasAntiDebugStr = strings.any {
            nameMatches(
                it,
                Regex(
                    "/proc/self/status|TracerPid|/proc/self/maps|/proc/%d/wchan|frida|xposed|substrate",
                    RegexOption.IGNORE_CASE
                )
            )
        }
        val hasRootDetect = strings.any {
            nameMatches(
                it,
                Regex(
                    "/system/app/Superuser|com\\.topjohnwu\\.magisk|su$|/su\\b|daemonsu|Magisk|SuperSU",
                    RegexOption.IGNORE_CASE
                )
            )
        }
        val hasEmulatorDetect = strings.any {
            nameMatches(
                it,
                Regex(
                    "goldfish|generic_x86|vbox86|nox|bluestacks|genymotion|android_x86|qemu\\.sf",
                    RegexOption.IGNORE_CASE
                )
            )
        }
        val hasSslPinning =
            allSymbols.any {
                nameMatches(
                    it.name,
                    Regex(
                        "ssl_verify|SSL_CTX_set_verify|X509_verify_cert|certificate_verify",
                        RegexOption.IGNORE_CASE
                    )
                )
            } ||
                strings.any {
                    nameMatches(
                        it,
                        Regex(
                            "pinning|CertificatePinner|ssl_error|TrustManager",
                            RegexOption.IGNORE_CASE
                        )
                    )
                }
        val hasDynLoad = allSymbols.any {
            it.name in
                setOf("dlopen", "dlsym", "dlclose", "android_dlopen_ext")
        }
        val hasDynRegister =
            allSymbols.any { it.name == "JNI_OnLoad" } || exported.any { it.name == "JNI_OnLoad" }
        val hasStrEncrypt =
            strings.count { it.length > 10 && it.matches(Regex("^[A-Za-z0-9+/=]{16,}$")) } > 10
        ...
        val hasOllvm =
            namedFunctions.count {
                nameMatches(
                    it.name,
                    Regex("\\.cold\\.|__clang_call_terminate|bcf\\.", RegexOption.IGNORE_CASE)
                )
            } >
                5 ||
                entropyGlobal > 7.2
        ...
        val visibility = if (totalFuncSymbols >
            0
        ) {
            ((exportedFuncs * 100.0) / totalFuncSymbols).roundToInt()
        } else {
            0
        }
        ...
        val security = JSONArray()
            .put(
                feature(
                    "pie",
                    isPie,
                    if (isPie) "PIE 启用" else "非 PIE (无随机化)",
                    if (isPie) "ok" else "danger",
            ...
            .put(
                feature(
                    "nx",
                    nxEnabled,
                    if (nxEnabled) "NX (DEP) 启用" else "栈可执行 (高危)",
                    if (nxEnabled) "ok" else "danger",
            ...
            .put(
                feature(
                    "canary",
                    hasCanary,
                    if (hasCanary) "Canary 栈保护" else "无 Canary",
                    if (hasCanary) "ok" else "danger",
            ...
            .put(
                feature(
                    "fortify",
                    hasFortify,
                    if (hasFortify) "FORTIFY_SOURCE" else "无 FORTIFY",
                    if (hasFortify) "ok" else "warn",
            ...
            .put(
                feature(
                    "cfi",
                    hasCfi,
                    if (hasCfi) "CFI 保护" else "未发现 CFI",
                    if (hasCfi) "ok" else "warn",
            ...
            .put(
                feature(
                    "anti_debug",
                    hasAntiDebugSym || hasAntiDebugStr,
                    if (hasAntiDebugSym ||
                        hasAntiDebugStr
                    ) {
                        "反调试"
            ...
            .put(
                feature(
                    "root",
                    hasRootDetect,
                    if (hasRootDetect) "Root检测" else "未检测Root",
                    if (hasRootDetect) "warn" else "ok",
            ...
            .put(
                feature(
                    "emulator",
                    hasEmulatorDetect,
                    if (hasEmulatorDetect) "模拟器检测" else "未检测模拟器",
                    if (hasEmulatorDetect) "warn" else "ok",
            ...
            .put(
                feature(
                    "dyn_load",
                    hasDynLoad,
                    if (hasDynLoad) "动态加载(dlopen)" else "无动态加载",
                    if (hasDynLoad) "warn" else "ok",
            ...
            .put(
                feature(
                    "ssl_pinning",
                    hasSslPinning,
                    if (hasSslPinning) "SSL Pinning" else "未检测SSL固定",
                    if (hasSslPinning) "warn" else "ok",
            ...
            .put(
                feature(
                    "ollvm",
                    hasOllvm,
                    if (hasOllvm) "疑似OLLVM混淆" else "未检测OLLVM",
                    if (hasOllvm) "danger" else "ok",
            ...
            .put(
                feature(
                    "str_encrypt",
                    hasStrEncrypt,
                    if (hasStrEncrypt) "疑似字符串加密" else "明文字符串",
                    if (hasStrEncrypt) "danger" else "ok",
            ...
            .put(
                feature(
                    "init_array",
                    hasInitArray,
                    if (hasInitArray) ".init_array 存在" else "无 .init_array",
                    if (hasInitArray) "warn" else "ok",
            ...
            .put(
                feature(
                    "stripped",
                    isStripped,
                    if (isStripped) "Stripped (已剥离)" else "符号表完整",
                    if (isStripped) "warn" else "ok",
            ...
            .put(
                feature(
                    "dwarf",
                    hasDwarf,
                    if (hasDwarf) "有DWARF调试" else "无调试信息",
                    if (hasDwarf) "ok" else "warn",
            ...
            .put(
                feature(
                    "textrel",
                    hasTextRel,
                    if (hasTextRel) "TEXTREL (代码段有写入)" else "无 TEXTREL",
                    if (hasTextRel) "danger" else "ok",
            ...
            neededArr.put(
                JSONObject()
                    ...
                    .put(
                        "description",
                        knownLibs[lib]
                            ?: knownLibs.entries.firstOrNull {
                                lib.startsWith(it.key.removeSuffix(".so"))
                            }?.value
                            ?: "未知自定义或第三方库"
                    )
            )
        ...
        val attackSurface = JSONArray()
            ...
            .put(
                attackItem(
                    "jni",
                    if (jniFuncs.isNotEmpty()) "danger" else "ok",
                    "JNI 函数入口",
                    "${jniFuncs.size}",
                    "（Java层调用Native的桥梁）"
                )
        ...
        if (hasDynRegister) {
            attackSurface.put(
                attackItem(
                    ...
                    "⚠ JNI_OnLoad 动态注册",
                    "",
                    "无法通过导出表直接找到Java方法对应的Native实现，需动态Hook跟踪"
                )
            )
        }
        if (hasInitArray) {
            attackSurface.put(
                attackItem(
                    ...
                    "⚠ .init_array 存在",
                    "",
                    "库加载时会先执行这些初始化函数，可能包含反调试/脱壳逻辑"
                )
            )
        }
        if (hasDynLoad) {
            attackSurface.put(
                attackItem("dlopen", "warn", "动态加载", "dlopen/dlsym", "运行时再加载代码路径，静态调用图可能不完整")
            )
        }
    ...
    private fun calcDifficulty(
        ...
        if (isStripped) {
            ...
            factors.put(
                factorItem(
                    ...
                    "danger",
                    "符号表被剥离 (Stripped)",
                    "内部 .symtab 缺失或为空，未导出函数失去原始名称，静态定位成本显著上升"
                )
            )
        }
        if (totallyStripped) {
            ...
            factors.put(
                factorItem(
                    ...
                    "danger",
                    "几乎无可用符号",
                    "连 dynsym 也稀少/缺失，函数边界与名称高度依赖反汇编与启发式恢复"
                )
            )
        }
        if (hasOllvm) {
            score += 3
            factors.put(
                factorItem("ollvm", 3.0, "danger", "疑似 OLLVM / 强混淆", "控制流平坦化或高熵代码特征会显著增加阅读与还原成本")
            )
        }
        if (hasStrEncrypt) {
            score += 2
            factors.put(
                factorItem(
                    ...
                    "danger",
                    "疑似字符串加密",
                    "大量高熵/类 Base64 数据会削弱字符串交叉引用这条最常用线索"
                )
            )
        }
        if (hasCfi) {
            score += 1
            factors.put(
                factorItem("cfi", 1.0, "warn", "CFI 控制流完整性", "间接调用校验会提高补丁与 Hook 的失败率，需要更精细的绕过")
            )
        }
        if (hasAntiDebug) {
            score += 1
            factors.put(
                factorItem(
                    ...
                    "danger",
                    "反调试机制",
                    "ptrace / TracerPid / Frida 特征扫描等会阻断常规动态分析流程"
                )
            )
        }
        if (hasRootDetect) {
            ...
            factors.put(
                factorItem(
                    ...
                    "warn",
                    "Root / Magisk 检测",
                    "可能在 Root 环境下主动降级或退出，影响 Frida/Xposed 方案"
                )
            )
        }
        if (hasSslPinning) {
            score += 0.5
            factors.put(
                factorItem("ssl_pinning", 0.5, "warn", "SSL Pinning", "抓包与协议还原前通常需要先绕过证书固定")
            )
        }
        ...
        if (entropyGlobal > 7.5) {
            ...
            factors.put(
                factorItem(
                    ...
                    "danger",
                    "超高熵值",
                    "全局抽样熵 ${"%.2f".format(entropyGlobal)}，更像加壳、加密资源或强混淆产物"
                )
            )
            ...
            factors.put(
                factorItem(
                    ...
                    "warn",
                    "偏高熵值",
                    "全局抽样熵 ${"%.2f".format(entropyGlobal)}，可能存在压缩段、混淆或加密数据"
                )
            )
        }
        ...
        return JSONObject()
            ...
            .put(
                "scoreGuide",
                "评分范围 0–10：<1 简单，1–3 较易，3–5 中等，5–7 困难，≥7 极难。分值由符号剥离、混淆、反调试、熵值、动态注册等加权累加。"
            )
    }
