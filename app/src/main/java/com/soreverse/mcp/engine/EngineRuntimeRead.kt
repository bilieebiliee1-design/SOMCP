...
internal fun EngineRuntime.list(
    workspaceId: String,
    ...
    val all = when (view) {
        "sections" -> elf.sections.asSequence().withIndex().filter {
            it.value.name.startsWith(prefix)
        }.map { (index, section) ->
            EngineJson.sectionJson(
                name,
                section,
                index
            ).put(
                "virtualAddr",
                hex(section.addr)
            ).put("fileOffset", hex(section.offset)).put("alignment", section.addralign)
        }
        ...
        "symbols", "dynsyms" -> (
            ...
            ).asSequence().filter { it.name.startsWith(prefix) }.map {
            EngineJson.symbolJson(name, it)
        }

        "functions" -> (elf.symbols + elf.dynSymbols).asSequence().filter {
            it.type == "FUNC" &&
                !it.imported &&
            ...
            JSONObject().put(
                ...
                "startAddr",
                hex(it.value and -2L)
            ).put("endAddr", hex((it.value and -2L) + it.size)).put("size", it.size).put(
                "section",
                sectionFor(elf, it.value)?.name ?: ""
            ).put("isExported", it.exported)
        }

        "relocations" -> elf.relocations.asSequence().filter {
            it.symbol.startsWith(prefix)
        }.map {
            EngineJson.relocationJson(name, elf, it)
        }

        "strings" -> elf.strings.asSequence().filter {
            prefix.isBlank() ||
                it.value.contains(prefix, ignoreCase = true)
        }.map {
            EngineJson.stringJson(name, it)
        }

        "imports" -> elf.dynSymbols.asSequence().filter {
            it.imported &&
                it.name.startsWith(prefix)
        }.map {
            JSONObject().put(
                "locator",
                "so_import:$name!UNRESOLVED!${it.name}"
            ).put(
                "soname",
                JSONObject.NULL
            ).put(
                "resolution",
                "unresolved_without_symbol_version_mapping"
            ).put(
                "neededLibraries",
                JSONArray(neededLibraries(elf, dataFor(workspaceId, editSessionId)))
            ).put("symbol", it.name).put("symbolLocator", "so_symbol:$name!${it.name}").put(
                "isWeak",
                it.bind == "WEAK"
            )
        }
...
internal fun EngineRuntime.readElf(workspaceId: String, editSessionId: String, pathHint: String = ""): JSONObject = guarded {
    ...
    ok(
        JSONObject()
            ...
            .put(
                "ident",
                JSONObject().put("magic", "7F 45 4C 46").put(
                    "class",
                    if (elf.bits ==
                        64
                    ) {
            ...
            .put(
                ...
                elf.sections.map {
                    JSONObject().put(
                        ...
                        "flags",
                        flags(it.flags)
                    ).put("addr", hex(it.addr)).put("offset", hex(it.offset)).put("size", it.size)
                }.toJsonArray()
            )
...
internal fun EngineRuntime.hexdump(workspaceId: String, editSessionId: String, locator: String, byteOffset: Int, maxBytes: Int): JSONObject = guarded {
    ...
    ok(
        JSONObject()
            ...
            .put(
                "targetLocator",
                EngineJson.sectionLocator(
                    workspaces[workspaceId]?.source?.name ?: "lib.so",
                    sec,
                    elf.sections.indexOf(sec)
                )
            )
            ...
            .put(
                ...
                ).put("byteOffset", byteOffset).put("bytesReturned", count)
            )
            .put("targetVersion", sha256(slice))
    )
}

internal fun EngineRuntime.strings(
    workspaceId: String,
    editSessionId: String,
    locator: String,
    prefix: String,
    ...
    val compiledRegex = if (regex && prefix.isNotBlank()) {
        runCatching {
            Regex(prefix, if (ignoreCase) setOf(RegexOption.IGNORE_CASE) else emptySet())
        }
            .getOrElse {
                return@guarded err(
                    "INVALID_REGEX",
                    it.message ?: "Invalid regex",
                    "prefix",
                    prefix
                )
            }
    ...
    val all = elf.strings.asSequence()
        ...
        .filter {
            encodingFilter.isBlank() ||
                it.encoding.equals(encodingFilter, ignoreCase = true) ||
                (encodingFilter == "UTF16" && it.encoding.startsWith("UTF-16", ignoreCase = true))
        }
        .filter { it.confidence >= minConfidence }
        .filter {
            prefix.isBlank() ||
                compiledRegex?.containsMatchIn(it.value)
                    ?: it.value.contains(prefix, ignoreCase = ignoreCase)
        }
...
internal fun EngineRuntime.disasm(
    workspaceId: String,
    editSessionId: String,
    locator: String,
    limit: Int,
    ...
    if (cursor.startsWith("disasm:") &&
        ...
        return@guarded err("INVALID_CURSOR", "Invalid disassembly cursor", "cursor", cursor)
    }
    ...
    val directRawVa = LocatorParser.hex(
        addr.ifBlank {
            effectiveLocator.takeIf { it.startsWith("0x", ignoreCase = true) }
                ?: ""
        }
    )
    ...
    if (directVa != null) {
        val directThumb =
            thumb ?: (mode == "thumb") ||
                (mode == "auto" && elf.architecture == "arm32" && (directRawVa and 1L) == 1L)
        val addressIssue = validateCodeAddress(elf, directVa, directThumb)
        if (addressIssue != null) return@guarded addressIssue
        val off =
            vaToOffset(elf, directVa)?.toInt()
                ?: return@guarded err(
                    "OFFSET_OUT_OF_RANGE",
                    "Address ${hex(directVa)} cannot be mapped to a file offset",
                    "addr",
                    hex(directVa)
                )
        ...
        val rizinText = runCatching {
            NativeEngine.active().disassemble(
                slice,
                elf.architecture,
                directVa,
                directThumb,
                effectiveLimit.coerceIn(1, 5000)
            )
        }.getOrDefault("")
        val referenceHints =
            disasmReferenceHints(effectiveWorkspaceId, effectiveEditSessionId, elf)
        val symbolHints = disasmSymbolHints(referenceHints, elf, directVa, windowBytes.toLong())
        val lines =
            annotateDisasmLines(
                referenceHints,
                cleanDisasmLines(rizinText, effectiveLimit.coerceIn(1, 5000))
            )
        ...
        return@guarded ok(
            JSONObject()
                ...
                .put(
                    ...
                    section?.let {
                        EngineJson.sectionJson(
                            workspaces[effectiveWorkspaceId]?.source?.name ?: "lib.so",
                            it,
                            elf.sections.indexOf(it)
                        )
                    }
                        ?: JSONObject.NULL
                )
                .put(
                    "resolvedFunction",
                    containingFunction?.let { functionIdentityJson(elf, it) } ?: JSONObject.NULL
                )
                ...
                .put(
                    "pseudocode",
                    decompileWithContext(
                        effectiveWorkspaceId,
                        effectiveEditSessionId,
                        elf,
                        directVa
                    )
                )
    ...
    val locatorVa =
        LocatorParser.address(effectiveLocator)
            ?: resolveRizinFunctionAddress(
                bytes,
                elf,
                LocatorParser.target(effectiveLocator, "so_function")
            )
    ...
    if (sym == null && locatorVa != null) {
        ...
        if (addressIssue != null) return@guarded addressIssue
        val off =
            vaToOffset(elf, locatorVa)?.toInt()
                ?: return@guarded err(
                    "OFFSET_OUT_OF_RANGE",
                    "Address ${hex(locatorVa)} cannot be mapped to a file offset",
                    "locator",
                    effectiveLocator
                )
        val windowLimit =
            rizinFunctionSize(bytes, elf, locatorVa)
                ?: functionByteSizeFromAddress(elf, locatorVa, off, bytes.size)
        val requestedMaxBytes = effectiveMaxBytes.coerceIn(1, 65536)
        ...
        val referenceHints =
            disasmReferenceHints(effectiveWorkspaceId, effectiveEditSessionId, elf)
        val symbolHints =
            disasmSymbolHints(referenceHints, elf, locatorVa, windowBytes.toLong())
        val lines =
            annotateDisasmLines(
                referenceHints,
                cleanDisasmLines(rizinText, effectiveLimit.coerceIn(1, 5000))
        ...
        return@guarded ok(
            JSONObject()
                ...
                .put(
                    "resolvedSection",
                    executableSectionFor(elf, locatorVa)?.let {
                        EngineJson.sectionJson(
                            workspaces[effectiveWorkspaceId]?.source?.name ?: "lib.so",
                            it,
                            elf.sections.indexOf(it)
                ...
                .put(
                    ...
                    JSONObject().put(
                        ...
                        "endAddr",
                        hex(locatorVa + windowLimit)
                    ).put("size", windowLimit).put("source", "rizin-locator-address")
                )
                .put(
                    "textWindow",
                    JSONObject().put(
    ...
    if (sym ==
        ...
        return@guarded err("FUNCTION_NOT_FOUND", "Function not found", "locator", locator)
    }
    val startAddress = sym.value and -2L
    val symThumb =
        thumb ?: (mode == "thumb") ||
            (mode == "auto" && elf.architecture == "arm32" && (sym.value and 1L) == 1L)
    val addressIssue = validateCodeAddress(elf, startAddress, symThumb)
    if (addressIssue != null) return@guarded addressIssue
    val off =
        vaToOffset(elf, startAddress)?.toInt()
            ?: return@guarded err("OFFSET_OUT_OF_RANGE", "Address cannot be mapped")
    val size = (
        rizinFunctionSize(bytes, elf, startAddress)
            ?: functionByteSize(elf, sym, off, bytes.size)
        ).coerceIn(0, bytes.size - off)
    val derivedByteOffset =
        cursorArgs?.byteOffset
            ?: if (byteOffset >
                0
            ) {
                byteOffset
            } else {
                instructionOffsetToByteOffset(elf, sym, instructionOffset)
            }
            ...
            JSONObject()
                ...
                .put(
                    ...
                    JSONObject().put(
                        ...
                        "endByteOffset",
                        windowOffset
                    ).put("startAddress", hex(startAddress + windowOffset)).put("maxBytes", 0)
                )
                .put("basicBlocks", JSONArray())
    ...
    val symbolHints =
        disasmSymbolHints(referenceHints, elf, windowAddress, windowBytes.toLong())
    val lines = if (usedPseudo) {
        text.lineSequence().take(
            effectiveLimit.coerceIn(1, 5000)
        ).toList()
    } else {
        annotateDisasmLines(
            referenceHints,
            cleanDisasmLines(text, effectiveLimit.coerceIn(1, 5000))
        )
    }
    val nextByteOffset = windowOffset + windowBytes
    val nextCursor = if (nextByteOffset <
        size
    ) {
        DisasmCursorCodec.encode(
            effectiveWorkspaceId,
            effectiveEditSessionId,
            effectiveLocator,
            nextByteOffset,
        ...
        JSONObject()
            ...
            .put(
                ...
                JSONObject().put(
                    "startAddr",
                    hex(startAddress)
                ).put("endAddr", hex(startAddress + size)).put("size", size).put("source", "symbol")
            )
            .put(
                "textWindow",
                JSONObject()
...
internal fun EngineRuntime.outline(workspaceId: String, editSessionId: String, locator: String, limit: Int): JSONObject = guarded {
    ...
    val sym =
        (elf.symbols + elf.dynSymbols).firstOrNull { it.name == name }
            ?: return@guarded err(
                "FUNCTION_NOT_FOUND",
                "Function not found",
                "locator",
    ...
    ok(
        JSONObject()
            ...
            .put(
                "instructionCount",
                if (elf.architecture in
                    setOf("arm32", "arm64", "mips")
                ) {
                    (sym.size / 4).toInt()
                } else {
                    sym.size.toInt()
            ...
            .put(
                ...
                JSONArray().put(
                    JSONObject().put(
                        ...
                        "isExit",
                        true
                    ).put("successors", JSONArray()).put("predecessors", JSONArray())
                )
            )
    ...
    elf.relocations.asSequence().filter { it.symbol == symbol }.take(limit).forEach {
        incoming.put(
            JSONObject().put(
                ...
                "from",
                "so_reloc:${workspaces[workspaceId]?.source?.name}!${it.section}!${it.offset.toString(
                    16
                )}"
            ).put("to", locator).put("address", hex(it.offset)).put("confidence", "high")
        )
    }
    ok(
        JSONObject()
...
internal fun EngineRuntime.xrefString(workspaceId: String, editSessionId: String, locator: String, limit: Int): JSONObject = guarded {
    ...
    val targetOffset =
        locator.substringAfterLast('!').toLongOrNull(16)
            ?: return@guarded err("INVALID_LOCATOR", "Invalid string locator")
    val targetVa = elf.sections.firstOrNull {
        targetOffset >= it.offset &&
            targetOffset < it.offset + it.size
    }?.let { it.addr + (targetOffset - it.offset) }
    val refs = JSONArray()
    if (targetVa != null) {
        val bytes = dataFor(workspaceId, editSessionId)
        val pattern =
            byteArrayOf(
                (targetVa and 0xff).toByte(),
                ((targetVa shr 8) and 0xff).toByte(),
                (
                    (
                        targetVa shr
                            16
                ...
                ((targetVa shr 24) and 0xff).toByte()
            )
        var i = 0
        while (i <= bytes.size - pattern.size && refs.length() < limit) {
            if (pattern.indices.all {
                    bytes[i + it] == pattern[it]
                }
            ) {
                refs.put(
                    JSONObject().put(
                        ...
                        "address",
                        hex(i.toLong())
                    ).put("locator", locator).put("confidence", "medium")
                )
            }
            i++
        }
    ...
    ok(
        JSONObject().put(
            ...
            "references",
            refs
        ).put("pagination", pagination(false, null, refs.length(), limit, refs.length()))
    )
}

internal fun EngineRuntime.search(
    workspaceId: String,
    editSessionId: String,
    target: String,
    query: String,
                ...
                .forEach {
                    next +=
                        JSONObject().put(
                            "target",
                            "symbols"
                        ).put("locator", "so_symbol:$name!${it.name}").put("snippet", it.name)
                }
        }
                ...
                .forEach {
                    next +=
                        JSONObject().put(
                            ...
                            "locator",
                            "so_string:$name!${it.offset.toString(16)}"
                        ).put("snippet", it.value.take(160))
                }
            rawUtf8Hits(
                dataFor(resolvedWorkspaceId, editSessionId),
                query,
                limit
            ).forEach { (offset, snippet) ->
                next +=
                    JSONObject().put(
                        "target",
                        "raw_utf8"
                        ...
                        "locator",
                        "so_raw_string:$name!${offset.toString(16)}"
                    ).put("snippet", snippet)
            }
        }
        next
...
internal fun EngineRuntime.readStats(workspaceId: String, editSessionId: String = "", pathHint: String = ""): JSONObject = guarded {
    ...
    val data = if (editSessionId.isNotBlank()) {
        ...
        workspaces[resolvedId]?.data
            ?: return@guarded err("WORKSPACE_NOT_FOUND", "Workspace not found")
    }
    val elf = runCatching { lief.parse(data) }.getOrElse {
        return@guarded err(
            "ELF_PARSE_FAILED",
    ...
    val symbolFunctions = (elf.symbols + elf.dynSymbols).count {
        it.type == "FUNC" &&
            !it.imported
    }
    ...
    val jniExports = (elf.symbols + elf.dynSymbols).count {
        it.name == "JNI_OnLoad" ||
            it.name.startsWith("Java_")
    }
    val res = JSONObject()
        ...
        .put(
            ...
            JSONObject()
                ...
                .put(
                    "functionsFieldMeaning",
                    "functions==analyzedFunctions (Rizin recovered routines including anonymous/local); symbolFunctions are ELF FUNC symbols; exportedFunctions are dynamic exports; pltStubs are import stubs"
                )
        ...
        .put(
            "hint",
            "Use analyze_elf (view=list, subView=<count-name>) for details. analyze_crypto for security-tinged hints. functionsMeaning explains which function count is default."
        )
    if (editSessionId.isNotBlank()) res.put("editSessionId", editSessionId)
    ok(res)
}

internal fun EngineRuntime.assembleRaw(
    workspaceId: String,
    editSessionId: String = "",
    asm: String,
    addr: Long = 0L,
    ...
    val useThumb =
        thumb ?: (mode == "thumb") ||
            (mode == "auto" && elf.architecture == "arm32" && (rawAddr and 1L) == 1L)
    val encoded = NativeEngine.active().assemble(
        asm,
        elf.architecture,
        normalizedAddr,
        useThumb
...
internal fun EngineRuntime.analyze(workspaceId: String, editSessionId: String, pathHint: String = ""): JSONObject = guarded {
    ...
    val registerNativesHints = (names + strings).filter {
        it.contains("RegisterNatives", ignoreCase = true)
    }.distinct().take(50)
    val urlStrings = strings.filter {
    ...
    val commandStrings = strings.filter { value ->
        listOf("su", "sh ", "chmod", "mount", "getprop", "setprop").any {
    ...
    val dynamicImports = imports.filter {
        it in
            setOf("dlopen", "dlsym", "dlclose", "android_dlopen_ext")
    }.distinct()
    val symbolFunctions = allSymbols.filter { it.type == "FUNC" && !it.imported }.distinctBy {
        it.name to
            it.value
    }
...
internal fun EngineRuntime.resolveCodeAddress(bytes: ByteArray, elf: ElfFile, locator: String, explicitAddress: String = ""): Long? {
    ...
    val name = LocatorParser.target(
        target,
        if (target.startsWith("so_symbol:")) "so_symbol" else "so_function"
    )
...
internal fun EngineRuntime.searchKey(workspaceId: String, editSessionId: String, target: String, query: String, limit: Int): String {
    val revision = if (editSessionId.isBlank()) {
        ...
        workspaces[workspaceId]?.edits?.get(editSessionId)?.revision
            ?: 0
    }
    return "$workspaceId|$editSessionId|$revision|$target|${query.lowercase()}|${limit.coerceIn(
        1,
        5000
    )}"
}
...
internal fun EngineRuntime.pageJson(slice: PageStore.PageSlice): JSONObject = ok(
    JSONObject()
...
internal fun EngineRuntime.functionByteSize(elf: ElfFile, sym: SymbolInfo, fileOffset: Int, fileSize: Int): Int {
    ...
    val bySection = sectionFor(elf, start)?.let {
        (it.offset + it.size - fileOffset).toInt().takeIf { size ->
            size >
                0
        }
    }
...
internal fun EngineRuntime.validateCodeAddress(elf: ElfFile, va: Long, thumb: Boolean): JSONObject? {
    ...
    if (alignment > 1 &&
        ...
        return err(
            ...
            "Address ${hex(
                va
            )} is not aligned for ${elf.architecture}${if (thumb) " thumb" else ""}",
            "addr",
            hex(va),
    ...
    val section =
        sectionFor(elf, va)
            ?: return err(
                "OFFSET_OUT_OF_RANGE",
                "Address ${hex(va)} does not belong to any section",
                "addr",
                hex(va)
            )
    if (!sectionExecutable(section)) {
        return err(
            "NON_EXECUTABLE_ADDRESS",
            "Address ${hex(va)} is in non-executable section '${section.name}'",
            "addr",
            hex(va),
            "section" to section.name,
            "sectionFlags" to flags(section.flags)
        )
    }
    return null
}
...
internal fun EngineRuntime.pseudoDisasm(
    elf: ElfFile,
    sym: SymbolInfo,
    bytes: ByteArray,
    limit: Int,
    startAddress: Long = sym.value and
        -2L
): String {
    val step = if (elf.architecture == "arm32" &&
        (sym.value and 1L) == 1L
    ) {
        2
    ...
    while (p < bytes.size && lines.size < limit) {
        ...
        lines +=
            "0x${(startAddress + p).toString(16)}: ${chunk.joinToString(" ") {
                "%02X".format(it)
            }}    .byte ${chunk.joinToString(", ") { "0x%02x".format(it) }}"
        p += step
    }
...
internal fun EngineRuntime.annotateDisasmLines(hints: Map<Long, String>, lines: List<String>): List<String> {
    ...
    return lines.map { line ->
        ...
        if (importNames.isEmpty()) {
            ...
            "$line ; import: ${importNames.joinToString(", ") {
                "sym.imp.$it"
            }}"
        }
    }
...
internal fun EngineRuntime.importAddressHints(elf: ElfFile): Map<Long, String> {
    val importedSymbols = elf.dynSymbols.filter {
        it.imported && it.name.isNotBlank()
    }.associateBy { it.name }
    val hints = linkedMapOf<Long, String>()
    elf.relocations
...
internal fun EngineRuntime.signalImports(elf: ElfFile): List<String> {
    val signalNames =
        setOf("signal", "sigaction", "sigprocmask", "sigfillset", "sigemptyset", "sigaddset", "pthread_sigmask", "kill", "raise")
...
internal fun EngineRuntime.flags(flags: Long) = buildString {
    if (flags and 1L !=
        0L
    ) {
        append('W')
    }
    if (flags and 2L != 0L) append('A')
    if (flags and 4L != 0L) append('X')
}
...
internal fun EngineRuntime.indexOf(data: ByteArray, pattern: ByteArray, startAt: Int): Int {
    if (pattern.isEmpty() || data.size < pattern.size) return -1
    for (i in startAt.coerceAtLeast(0)..data.size - pattern.size) {
        if (pattern.indices.all {
                data[
                    i +
                        it
...
internal fun EngineRuntime.inferReturnType(bytes: ByteArray, elf: ElfFile, startVa: Long, size: Int): JSONObject {
    if (elf.architecture != "arm64" ||
        ...
        return JSONObject().put("returnType", "unknown").put("confidence", 0.0)
    }
    val off =
    ...
    while (i + 4 <= end) {
        ...
        // TBZ/TBNZ W0, #0, ...
        if ((insn and 0xff00001f.toInt()) == 0x36000000 ||
            (insn and 0xff00001f.toInt()) == 0x37000000
        ) {
    ...
    return when {
        sawZero && sawOne -> JSONObject().put(
            "returnType",
            "bool|int"
        ).put(
            "confidence",
            if (sawBoolBranch) 0.86 else 0.72
        ).put(
            "evidence",
            JSONArray(
                listOfNotNull(
                    "mov w0,#0",
                    "mov w0,#1",
                    if (sawBoolBranch) "tbz/tbnz w0,#0" else null
                )
            )
        )

        sawBoolBranch -> JSONObject().put(
            "returnType",
            "bool|int"
        ).put("confidence", 0.6).put("evidence", JSONArray(listOf("tbz/tbnz w0,#0")))
