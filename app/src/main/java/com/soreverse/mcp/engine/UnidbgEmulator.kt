...
class UnidbgEmulator(private val context: Context) {
    ...
    companion object {
        ...
        fun ensureNativeLibraries(): Boolean = synchronized(this) {
            ...
            runCatching {
                ...
                listOf("disassembler", "demumble").forEach {
                    runCatching { System.loadLibrary(it) }
                        .onFailure {
                            ...
                            AppLog.w(
                                "Unidbg optional native '$it' not bundled; continuing without it: ${it.message}"
                            )
                        }
                }
                ...
                runCatching {
                    val demangle = Class.forName(
                        "com.github.zhkl0228.demumble.Demangler"
                    ).getDeclaredMethod("demangle", String::class.java)
                    ...
                    optionalNativeMissing = true
                    AppLog.w(
                        "Unidbg optional demumble self-test failed; continuing without it: ${it.message}"
                    )
                }
        ...
        fun nativeDependencyErrorChain(): JSONArray {
            ...
            while (current != null && chain.length() < 8) {
                chain.put(
                    JSONObject().put("type", current.javaClass.name).put(
                        "message",
                        current.message ?: JSONObject.NULL
                    )
                )
    ...
    data class LiveSession(
        ...
        val closed: java.util.concurrent.atomic.AtomicBoolean = java.util.concurrent.atomic.AtomicBoolean(
            false
        )
    )
    ...
    fun emulate(bytes: ByteArray, arch: String, symbolName: String, argsJson: JSONArray, enableTrace: Boolean): JSONObject = runCatching {
        ...
        runCatching { doEmulate(bytes, arch, symbolName, argsJson, enableTrace, result) }
            .onFailure { e ->
                ...
                result.put("ok", false)
                    .put(
                        "error",
                        JSONObject().put("code", "EMULATION_ERROR")
                            ...
                            .put(
                                ...
                                JSONArray()
                                    .put(
                                        "Call analyze_elf(view=list, subView=dynsyms) to confirm the symbol is exported"
                                    )
                                    .put(
                                        "Try symbolName=JNI_OnLoad first to verify library initialization"
                                    )
                                    .put(
                                        "Use trace=true only for small functions because traces are verbose"
                                    )
                            )
                    )
            }
        ...
        JSONObject().put("ok", false)
            .put(
                "error",
                JSONObject().put("code", "EMULATION_ERROR").put(
                    "message",
                    e.message ?: "unknown"
                )
            )
    ...
    fun openSession(bytes: ByteArray, arch: String, callJniOnLoad: Boolean = true): JSONObject {
        ...
        return runCatching {
            ...
            result.put("ok", false)
                .put(
                    "error",
                    JSONObject().put("code", "SESSION_OPEN_ERROR")
                        .put("message", root.message ?: root.javaClass.simpleName)
                        .put("exception", e.javaClass.name)
                        .put("rootException", root.javaClass.name)
                        .put("stage", result.optString("stage", "unknown"))
    ...
    fun sessionMemoryMaps(live: LiveSession): JSONObject = runCatching {
        synchronized(live.lock) {
            ...
            val maps =
                memory.javaClass.getMethod("getMemoryMap").invoke(memory) as? Collection<*>
                    ?: emptyList<Any>()
            val items = JSONArray()
            maps.forEach { map ->
            ...
            JSONObject().put(
                ...
                "persistent",
                true
            ).put("addressSpace", "runtimeAbsoluteVirtualAddress").put("memoryMaps", items)
        }
    }.getOrElse { e ->
        JSONObject().put("ok", false).put(
            "error",
    ...
    fun sessionRegisters(live: LiveSession): JSONObject = runCatching {
        synchronized(live.lock) {
            val context =
                live.emulator.javaClass.getMethod("getContext").invoke(live.emulator)
                    ?: throw IllegalStateException("Register context unavailable")
            val values = if (live.arch == "arm64" ||
                live.arch == "x86_64"
            ) {
                readArm64Context(context)
            } else {
                readArm32Context(context)
            }
            JSONObject().put(
                "ok",
                true
                ...
                "registerScope",
                "generalPurposeSnapshot"
            ).put(
                "note",
                "This is a general-purpose register snapshot. SIMD/FP/system registers require explicit backend-specific support."
            ).put("registers", values)
        }
    }.getOrElse { e ->
        JSONObject().put("ok", false).put(
            "error",
            JSONObject().put("code", "REGISTERS_ERROR").put(
                "message",
                rootCause(e).message ?: e.javaClass.simpleName
            )
        )
    ...
    fun sessionMemoryUnmap(live: LiveSession, addr: Long, size: Long): JSONObject = runCatching {
        synchronized(live.lock) {
            ...
            val memUnmap =
                memory.javaClass.methods.firstOrNull {
                    it.name == "munmap" && it.parameterCount == 2
                }
                    ?: return@runCatching JSONObject().put(
                        "ok",
                        false
                        ...
                        JSONObject().put(
                            "code",
                            "MEMORY_API_ERROR"
                        ).put("message", "munmap not available")
                    )
            ...
            JSONObject().put(
                "ok",
                true
            ).put("persistent", true).put("addr", "0x${addr.toString(16)}").put("size", size)
        }
    }.getOrElse { e ->
        JSONObject().put("ok", false).put(
            "error",
            JSONObject().put("code", "MEMORY_UNMAP_ERROR").put(
                "message",
                rootCause(e).message ?: e.javaClass.simpleName
            )
        )
    ...
    fun sessionModules(live: LiveSession): JSONObject = runCatching {
        synchronized(live.lock) {
            ...
            val modules =
                memory.javaClass.methods.firstOrNull {
                    it.name == "getLoadedModules" &&
                        it.parameterCount == 0
                }?.invoke(memory) as? Collection<*>
                    ?: listOf(live.module)
            val items = JSONArray()
        ...
        JSONObject().put("ok", false).put(
            "error",
            JSONObject().put("code", "MODULES_ERROR").put(
                "message",
                rootCause(e).message ?: e.javaClass.simpleName
            )
        )
    ...
    fun sessionExports(live: LiveSession): JSONObject = runCatching {
        synchronized(live.lock) {
            val symbols =
                live.module.javaClass.methods.firstOrNull {
                    it.name == "getExportedSymbols" &&
                        it.parameterCount == 0
                }?.invoke(live.module) as? Collection<*>
                    ?: emptyList<Any>()
            val items = JSONArray()
        ...
        JSONObject().put("ok", false).put(
            "error",
            JSONObject().put("code", "EXPORTS_ERROR").put(
                "message",
                rootCause(e).message ?: e.javaClass.simpleName
            )
        )
    ...
    fun sessionTraceCode(live: LiveSession, begin: Long, end: Long): JSONObject = runCatching {
        synchronized(live.lock) {
            val method =
                live.emulator.javaClass.methods.firstOrNull {
                    it.name == "traceCode" &&
                        it.parameterCount == 2
                }
                    ?: return@runCatching JSONObject().put(
                        ...
                        JSONObject().put(
                            "code",
                            "TRACE_UNAVAILABLE"
                        ).put("message", "traceCode(begin,end) not available")
                    )
            ...
            JSONObject().put(
                ...
                "trace",
                "code"
            ).put("begin", "0x${begin.toString(16)}").put("end", "0x${end.toString(16)}").put(
                "hook",
                hook?.javaClass?.name ?: JSONObject.NULL
            )
        }
    }.getOrElse { e ->
        JSONObject().put("ok", false).put(
    ...
    fun sessionTraceStop(live: LiveSession, traceId: String): JSONObject = runCatching {
        synchronized(live.lock) {
            val hook =
                live.traceHooks.remove(traceId)
                    ?: throw IllegalArgumentException("Trace hook not found")
            hook.javaClass.methods.first {
                it.name == "stopTrace" && it.parameterCount == 0
            }.invoke(hook)
            JSONObject().put(
                "ok",
                true
            ).put("persistent", true).put("traceId", traceId).put("stopped", true)
        }
    }.getOrElse { e ->
        JSONObject().put("ok", false).put(
            "error",
    ...
    fun sessionHookStop(live: LiveSession, hookId: String): JSONObject = runCatching {
        synchronized(live.lock) {
            val hook =
                live.traceHooks.remove(hookId) ?: throw IllegalArgumentException("Hook not found")
            val unhook = hook.javaClass.methods.firstOrNull {
                it.name == "unhook" &&
                    it.parameterCount == 0
            }
            val detach = hook.javaClass.methods.firstOrNull {
                it.name == "detach" &&
                    it.parameterCount == 0
            }
            when {
            ...
            JSONObject().put(
                "ok",
                true
            ).put("persistent", true).put("hookId", hookId).put("stopped", true)
        }
    }.getOrElse { e ->
        JSONObject().put("ok", false).put(
            "error",
    ...
    fun sessionHookList(live: LiveSession): JSONObject = synchronized(live.lock) {
        JSONObject().put(
            "ok",
            true
        ).put("persistent", true).put("hooks", JSONArray(live.traceHooks.keys))
    }
    ...
    fun sessionBreakpointAdd(live: LiveSession, address: Long): JSONObject = runCatching {
        synchronized(live.lock) {
            val debugger =
                live.emulator.javaClass.methods.firstOrNull {
                    ...
                    ?: return@runCatching JSONObject().put(
                        ...
                        ).put("message", "attach() returned null")
                    )
            val breakpoint = debugger.javaClass.methods.firstOrNull {
                it.name == "addBreakPoint" &&
                    it.parameterCount == 1 &&
                    it.parameterTypes[0] == Long::class.javaPrimitiveType
            }?.invoke(debugger, address)
            ...
            JSONObject().put(
                ...
                "persistent",
                true
            ).put("addr", "0x${address.toString(16)}").put("breakpointId", breakpointId).put(
                "backendHandleType",
                breakpoint?.javaClass?.name ?: JSONObject.NULL
            )
        }
    }.getOrElse { e ->
        JSONObject().put("ok", false).put(
            "error",
            JSONObject().put("code", "BREAKPOINT_ERROR").put(
                "message",
                rootCause(e).message ?: e.javaClass.simpleName
            )
        )
    ...
    fun sessionDebuggerStatus(live: LiveSession): JSONObject = runCatching {
        synchronized(live.lock) {
            ...
            val points =
                debugger.javaClass.methods.first {
                    it.name == "getBreakPoints" &&
                        it.parameterCount == 0
                }.invoke(debugger) as? Map<*, *>
                    ?: emptyMap<Any, Any>()
            val items = JSONArray()
            points.forEach { (address, point) ->
                items.put(
                    JSONObject().put(
                        "address",
                        "0x${(address as Number).toLong().toString(16)}"
                    ).put(
                        "temporary",
                        point?.javaClass?.methods?.firstOrNull {
                            it.name == "isTemporary"
                        }?.invoke(point)
                            ?: false
                    ).put(
            ...
            JSONObject().put("ok", true).put("persistent", true).put(
                "isDebugging",
                debugger.javaClass.methods.first {
                    it.name ==
                        "isDebugging"
            ...
            ).put("breakpoints", items)
        }
    }.getOrElse { e ->
        JSONObject().put("ok", false).put(
            "error",
            JSONObject().put("code", "DEBUGGER_ERROR").put(
                "message",
                rootCause(e).message ?: e.javaClass.simpleName
            )
        )
    ...
    fun sessionBreakpointRemove(live: LiveSession, address: Long): JSONObject = runCatching {
        synchronized(live.lock) {
            val debugger = debugger(live)
            val removed = debugger.javaClass.methods.first {
                it.name == "removeBreakPoint" &&
                    it.parameterCount == 1
            }.invoke(debugger, address) as Boolean
            ...
            JSONObject().put(
                ...
                "persistent",
                true
            ).put("removed", removed).put("address", "0x${address.toString(16)}")
        }
    }.getOrElse { e ->
        JSONObject().put("ok", false).put(
            "error",
    ...
    fun sessionSingleStep(live: LiveSession, count: Int): JSONObject = runCatching {
        synchronized(live.lock) {
            val backend = live.backend ?: throw IllegalStateException("Backend unavailable")
            backend.javaClass.methods.first {
                it.name == "setSingleStep" && it.parameterCount == 1
            }.invoke(backend, count.coerceAtLeast(1))
            JSONObject().put(
                "ok",
                true
            ).put("persistent", true).put("singleStepCount", count.coerceAtLeast(1))
        }
    }.getOrElse { e ->
        JSONObject().put("ok", false).put(
            "error",
            JSONObject().put("code", "SINGLE_STEP_ERROR").put(
                "message",
                rootCause(e).message ?: e.javaClass.simpleName
            )
        )
    ...
    fun sessionEmuStop(live: LiveSession): JSONObject = runCatching {
        synchronized(live.lock) {
            val backend = live.backend ?: throw IllegalStateException("Backend unavailable")
            backend.javaClass.methods.first {
                it.name == "emu_stop" && it.parameterCount == 0
            }.invoke(backend)
        ...
        JSONObject().put("ok", false).put(
            "error",
            JSONObject().put("code", "EMU_STOP_ERROR").put(
                "message",
                rootCause(e).message ?: e.javaClass.simpleName
            )
        )
    ...
    private fun debugger(live: LiveSession): Any = live.objects["debugger"]
        ?: (
            live.emulator.javaClass.methods.first {
                it.name == "attach" &&
                    it.parameterCount == 0
            }.invoke(live.emulator)
                ?: throw IllegalStateException("Debugger unavailable")
            ).also {
            live.objects["debugger"] =
                it
        }
    ...
    fun sessionReflectRoots(live: LiveSession): JSONObject = synchronized(live.lock) {
        ...
        runCatching {
            live.emulator.javaClass.methods.firstOrNull {
                it.name == "getSyscallHandler" &&
                    it.parameterCount == 0
            }?.invoke(live.emulator)
        }.getOrNull()?.let {
            live.objects["syscallHandler"] =
                it
        }
        live.objects["debugger"] = debugger(live)
        val roots = JSONObject()
        live.objects.forEach { (handle, value) ->
            roots.put(handle, JSONObject().put("handle", handle).put("class", value.javaClass.name))
        }
    ...
    fun sessionReflectInvoke(live: LiveSession, handle: String, methodName: String, args: JSONArray): JSONObject = synchronized(live.lock) {
        ...
        for (method in candidates) {
            val attempt = runCatching {
                val converted =
                    Array(method.parameterCount) { index ->
                        reflectArgument(live, args.opt(index), method.parameterTypes[index])
                    }
            ...
            failures.put(
                ...
                ).put("error", rootCause(attempt.exceptionOrNull()!!).toString())
            )
        }
        JSONObject().put(
            "ok",
            ...
            JSONObject().put(
                "code",
                "REFLECT_INVOKE_FAILED"
            ).put(
                "message",
                "No compatible public method overload succeeded"
            ).put("handle", handle).put("method", methodName).put("failures", failures)
        )
    }
    ...
    fun sessionNativeToolSchemas(live: LiveSession): JSONObject = synchronized(live.lock) {
        runCatching {
            ...
            JSONObject().put(
                ...
                "persistent",
                true
            ).put("source", "unidbg-upstream-mcp").put("schemas", JSONArray(schemas.toString()))
        }.getOrElse { error ->
            JSONObject().put(
                "ok",
                false
                ...
                JSONObject().put(
                    "code",
                    "UNIDBG_NATIVE_MCP_UNAVAILABLE"
                ).put("message", rootCause(error).toString())
            )
        }
    }
    ...
    private fun nativeMcpTools(live: LiveSession): Any {
        ...
        val server = serverClass.getConstructor(
            emulatorClass,
            Int::class.javaPrimitiveType
        ).newInstance(live.emulator, 0)
        val toolsClass = Class.forName("com.github.unidbg.mcp.McpTools")
        val tools = toolsClass.getConstructor(
            emulatorClass,
            serverClass
        ).newInstance(live.emulator, server)
    ...
    private fun reflectArgument(live: LiveSession, value: Any?, type: Class<*>): Any? {
        ...
        if (value is JSONObject &&
            ...
            return live.objects[value.getString("handle")]
                ?: throw IllegalArgumentException("Unknown object handle")
        }
    ...
    private fun reflectResult(live: LiveSession, value: Any?): Any {
        ...
        if (value.javaClass.isArray) {
            return JSONArray(
                (
                    ...
                    reflectResult(live, java.lang.reflect.Array.get(value, it))
                }
            )
        }
        ...
        if (value is Map<*, *>) {
            return JSONObject().also { json ->
                value.forEach { (key, item) -> json.put(key.toString(), reflectResult(live, item)) }
            }
        }
        ...
        return JSONObject().put(
            "handle",
            handle
        ).put("class", value.javaClass.name).put("display", value.toString())
    }
    ...
    private fun createLiveSession(bytes: ByteArray, arch: String, callJniOnLoad: Boolean, out: JSONObject): LiveSession {
        ...
        val builder = emulatorBuilderCls.getDeclaredMethod(
            if (for64Bit) "for64Bit" else "for32Bit"
        ).invoke(null)!!
        ...
        val dalvikModule = vm?.javaClass?.methods?.firstOrNull {
            it.name == "loadLibrary" &&
                it.parameterCount >= 2
        }?.invoke(vm, file, false)
    ...
    private fun callJniOnLoad(live: LiveSession) {
        val findSym = live.module.javaClass.getMethod(
            "findSymbolByName",
            String::class.java,
        ...
        if (jniOnLoad != null) {
            runCatching { callSymbol(jniOnLoad, live.emulator, listOf(live.vm)) }
    ...
    private fun doLiveCall(live: LiveSession, symbolName: String, argsJson: JSONArray, enableTrace: Boolean, out: JSONObject) {
        if (enableTrace &&
            ...
            runCatching {
                live.vm.javaClass.methods.firstOrNull {
                    it.name ==
                        "setVerbose" &&
                        it.parameterCount == 1
                }?.invoke(live.vm, true)
        ...
        if (symbol == null) {
            out.put(
                ...
                JSONObject().put(
                    "code",
                    "SYMBOL_NOT_FOUND"
                ).put("message", "Symbol $symbolName not exported by the SO")
            )
            return
        }
    ...
    private fun setAndroidResolver(emulator: Any) {
        ...
        val resolver = resolverCls.getConstructor(
            Int::class.javaPrimitiveType,
    ...
    private fun readLiveMemory(live: LiveSession, addr: Long, size: Int, out: JSONObject) {
        ...
        val memRead = live.backend?.javaClass?.methods?.firstOrNull {
            it.name == "mem_read" &&
                it.parameterCount == 2
        }
        if (memRead == null) {
            out.put(
                ...
                JSONObject().put(
                    "code",
                    "BACKEND_ERROR"
                ).put("message", "mem_read not available on backend")
            )
            return
        }
        ...
        if (raw == null || raw.isEmpty()) {
            out.put(
                ...
                "error",
                JSONObject().put("code", "READ_FAILED").put("message", "Memory read returned empty")
            )
            return
        }
        val hex = raw.joinToString("") { "%02x".format(it) }
        val ascii = raw.joinToString("") {
            if (it in
                32..126
            ) {
    ...
    private fun readArm64Context(context: Any): JSONObject {
        ...
        val getXLong = context.javaClass.methods.firstOrNull {
            it.name == "getXLong" &&
                it.parameterCount == 1
        }
        for (i in 0..28) {
            runCatching { getXLong?.invoke(context, i) }
                ...
                .onFailure {
                    values.put(
                        ...
                        JSONObject().put(
                            "error",
                            rootCause(it).message ?: it.javaClass.simpleName
                        )
                    )
                }
        }
        listOf(
            "getFp" to "fp",
            "getLR" to "lr",
            "getStackPointer" to "sp",
            "getPCPointer" to "pc"
        ).forEach { (methodName, name) ->
            runCatching {
                context.javaClass.methods.firstOrNull {
                    it.name == methodName &&
                        it.parameterCount == 0
                }?.invoke(context)
            }
    ...
    private fun pointerValue(value: Any?): String = when (value) {
        ...
        is Number -> value.toString()
            ...
            value.javaClass.getMethod("peer").invoke(value)?.toString()
        }.getOrNull()
            ?: value.toString()
    }
    ...
    private fun moduleJson(module: Any): JSONObject {
        ...
        return JSONObject()
            ...
            .put(
                "path",
                runCatching {
                    cls.getMethod("getPath").invoke(module)?.toString()
                }.getOrDefault("")
            )
            .put(
                "fileSize",
                runCatching {
                    cls.getMethod("getFileSize").invoke(module)
    ...
    private fun symbolJson(symbol: Any): JSONObject {
        ...
        val address =
            runCatching {
                cls.getMethod("getAddress").invoke(symbol) as? Number
            }.getOrNull()?.toLong()
                ?: 0L
        val value =
            runCatching {
                cls.getMethod("getValue").invoke(symbol) as? Number
            }.getOrNull()?.toLong()
                ?: 0L
        return JSONObject()
            .put(
                "name",
                runCatching {
                    cls.getMethod("getName").invoke(symbol)?.toString()
                }.getOrDefault(symbol.toString())
            )
            .put(
                "module",
                runCatching {
                    cls.getMethod("getModuleName").invoke(symbol)?.toString()
                }.getOrDefault("")
            )
            ...
            .put(
                "undef",
                runCatching {
                    cls.getMethod("isUndef").invoke(symbol) as? Boolean
                }.getOrDefault(false)
            )
    }
    ...
    private fun rootCause(error: Throwable): Throwable {
        var current = if (error is InvocationTargetException &&
    ...
    private fun toArgList(argsJson: JSONArray): List<Any?> = (0 until argsJson.length()).map { i ->
    ...
    private fun addSuspiciousReturnWarning(result: JSONObject, symbolName: String, traced: Boolean) {
        ...
        result.put("resultConfidence", "suspicious")
            .put(
                "semanticWarning",
                "Unidbg completed without a backend exception, but returnValue=-1 is suspicious for JNI boolean/int probes such as signal/setjmp guards. Treat this as an emulation-result warning, not proof of target logic success."
            )
            .put(
                "diagnostics",
                JSONObject()
                    ...
                    .put(
                        ...
                        JSONArray()
                            ...
                            .put(
                                "Add target-specific signal/sigsetjmp/siglongjmp/syscall hooks if the function relies on signal recovery"
                            )
                            .put(
                                "Use trace_code plus external log capture when instruction trace lines are required"
                            )
                    )
            )
    }
