...
internal fun EngineRuntime.unidbgDispatch(
    workspaceId: String,
    ...
    if (op.startsWith("session_") &&
        op !in setOf("session_open", "session_list", "session_close")
    ) {
        val live = emulatorSessions[args.optString(0)]?.live
        if (live != null && live.closed.get()) {
            return@guarded err(
                "SESSION_CLOSED",
                "Emulator session has been closed",
                "emulatorSessionId",
                args.optString(0)
            )
        }
    ...
    return@guarded when (op) {
        "status", "roots" -> ok(
            emulationStatus().put(
                "roots",
                JSONArray(
                    listOf("emulator", "memory", "vm", "module", "symbols", "jni", "framework", "hooks", "environment", "debugger")
                )
            )
        )
            ...
            JSONObject().put(
                ...
                JSONArray(
                    listOf(
                        JSONObject().put(
                            "name",
                            workspaces[workspaceId]?.source?.name ?: "target.so"
                        ).put(
                            "architecture",
                            elfFor(workspaceId, editSessionId).architecture
                        ).put("source", "static-workspace")
                    )
                )
            ).put(
                "runtime",
                "Call session_open + session_call to load through Unidbg; persistent runtime module listing requires live emulator sessions."
            )
        )
        ...
        "imports" -> list(
            ...
            "",
            500
        ).put("unidbgView", "imports")

        "debugger_plan", "memory_map_plan", "registers_plan", "breakpoints_plan", "trace_plan" -> ok(
            unidbgDebuggerPlan(op)
        )
        ...
        "session_open" -> {
            ...
            val open = unidbg.openSession(
                dataFor(workspaceId, editSessionId),
                elf.architecture,
                args.optBoolean(1, true)
            )
            if (!open.optBoolean(
                    "ok",
                    false
                ...
                return@guarded wrapUnidbgResult(
                    open.put("workspaceId", workspaceId).put("architecture", elf.architecture)
                )
            }
            val live = open.remove("live") as? UnidbgEmulator.LiveSession
                ?: return@guarded err(
                    "SESSION_OPEN_ERROR",
                    "Unidbg live session handle was not created"
                )
            emulatorSessions[id] =
                EmulatorSession(
                    id,
            ...
            ok(
                open.put(
                    ...
                    "editSessionId",
                    editSessionId
                ).put("architecture", elf.architecture).put("persistent", true)
            )
        }
                ...
                JSONArray(
                    emulatorSessions.values.map {
                        JSONObject().put(
                            ...
                            "editSessionId",
                            it.editSessionId
                        ).put("architecture", it.architecture).put("createdAt", it.createdAt)
                    }
                )
            )
        )
        ...
        "session_close" -> {
            ...
            ok(
                JSONObject().put(
                    "closed",
                    session != null
                ).put("emulatorSessionId", args.optString(0))
            )
        }

        "session_call" -> {
            val session =
                emulatorSessions[args.optString(0)]
                    ?: return@guarded err(
                        "EMULATOR_SESSION_NOT_FOUND",
                        "Emulator session not found",
                        "emulatorSessionId",
                        args.optString(0)
                    )
            ...
            wrapUnidbgResult(
                result.put(
                    ...
                    "workspaceId",
                    session.workspaceId
                ).put("symbolName", symbol).put("architecture", session.architecture)
            )
        }
        ...
        "session_call_address" -> {
            ...
            val addr =
                parseHexLong(args.optString(1))
                    ?: return@guarded err(
                        "INVALID_ARGUMENT",
                        "args[1] must be a hex address",
                        "args",
                        args
                    )
            val callArgs = args.optJSONArray(2) ?: JSONArray()
            val result = session.live?.let { unidbg.sessionCallAddress(it, addr, callArgs) }
                ?: JSONObject().put(
                    ...
                    ).put("message", "address calls require a live emulator session")
                )
            wrapUnidbgResult(
                result.put(
                    "emulatorSessionId",
                    session.id
                ).put("workspaceId", session.workspaceId).put("architecture", session.architecture)
            )
        }
            ...
            val result = session.live?.let { unidbg.sessionMemoryMaps(it) }
                ?: JSONObject().put(
                    ...
                    JSONObject().put(
                        "code",
                        "LIVE_SESSION_REQUIRED"
                    ).put("message", "memory maps require a live emulator session")
                )
            wrapUnidbgResult(
                result.put(
                    "emulatorSessionId",
            ...
            val result = session.live?.let { unidbg.sessionRegisters(it) }
                ?: JSONObject().put(
                    ...
                    JSONObject().put(
                        "code",
                        "LIVE_SESSION_REQUIRED"
                    ).put("message", "registers require a live emulator session")
                )
            wrapUnidbgResult(
                result.put(
                    "emulatorSessionId",
            ...
            val result = session.live?.let { unidbg.sessionModules(it) }
                ?: JSONObject().put(
                    ...
                    JSONObject().put(
                        "code",
                        "LIVE_SESSION_REQUIRED"
                    ).put("message", "modules require a live emulator session")
                )
            wrapUnidbgResult(
                result.put(
                    "emulatorSessionId",
            ...
            val result = session.live?.let { unidbg.sessionExports(it) }
                ?: JSONObject().put(
                    ...
                    JSONObject().put(
                        "code",
                        "LIVE_SESSION_REQUIRED"
                    ).put("message", "exports require a live emulator session")
                )
            wrapUnidbgResult(
                result.put(
                    "emulatorSessionId",
        ...
        "session_trace_code" -> {
            ...
            val begin =
                parseHexLong(args.optString(1))
                    ?: return@guarded err(
                        "INVALID_ARGUMENT",
                        "args[1] must be a hex begin address",
                        "args",
                        args
                    )
            val end =
                parseHexLong(args.optString(2))
                    ?: return@guarded err(
                        "INVALID_ARGUMENT",
                        "args[2] must be a hex end address",
                        "args",
                        args
                    )
            val result = session.live?.let { unidbg.sessionTraceCode(it, begin, end) }
                ?: JSONObject().put(
                    ...
                    JSONObject().put(
                        "code",
                        "LIVE_SESSION_REQUIRED"
                    ).put("message", "trace requires a live emulator session")
                )
            wrapUnidbgResult(
                result.put(
                    "emulatorSessionId",
        ...
        "session_trace_start" -> {
            ...
            wrapUnidbgResult(
                (
                    session.live?.let {
                        unidbg.sessionTraceStart(it, args.optString(1, "code"), begin, end)
                    }
                        ?: JSONObject().put(
                            "ok",
                            false
                        ).put("error", JSONObject().put("code", "LIVE_SESSION_REQUIRED"))
                    ).put("emulatorSessionId", session.id)
            )
        }
        ...
        "session_trace_events" -> {
            ...
            wrapUnidbgResult(
                (
                    session.live?.let {
                        unidbg.sessionTraceEvents(it, args.optInt(1, 0), args.optInt(2, 100))
                    }
                        ?: JSONObject().put(
                            "ok",
                            false
        ...
        "session_trace_stop" -> {
            ...
            wrapUnidbgResult(
                (
                    session.live?.let { unidbg.sessionTraceStop(it, args.optString(1)) }
                        ?: JSONObject().put(
                            "ok",
                            false
                        ).put("error", JSONObject().put("code", "LIVE_SESSION_REQUIRED"))
        ...
        "session_trace_clear" -> {
            ...
            wrapUnidbgResult(
                (
                    session.live?.let { unidbg.sessionTraceClear(it) }
                        ?: JSONObject().put(
                            "ok",
                            false
                        ).put("error", JSONObject().put("code", "LIVE_SESSION_REQUIRED"))
        ...
        "session_hook_start" -> {
            ...
            wrapUnidbgResult(
                (
                    session.live?.let {
                        unidbg.sessionHookStart(
                            it,
                            args.optString(1, "syscall"),
                            parseHexLong(args.optString(2)) ?: 1L,
                            parseHexLong(args.optString(3)) ?: 0L
                        )
                    }
                        ?: JSONObject().put(
                            "ok",
        ...
        "session_hook_list" -> {
            ...
            wrapUnidbgResult(
                (
                    session.live?.let { unidbg.sessionHookList(it) }
                        ?: JSONObject().put(
                            "ok",
                            false
                        ).put("error", JSONObject().put("code", "LIVE_SESSION_REQUIRED"))
        ...
        "session_hook_stop" -> {
            ...
            wrapUnidbgResult(
                (
                    session.live?.let { unidbg.sessionHookStop(it, args.optString(1)) }
                        ?: JSONObject().put(
                            "ok",
                            false
                        ).put("error", JSONObject().put("code", "LIVE_SESSION_REQUIRED"))
            ...
            val result = session.live?.let { unidbg.sessionBreakpointAdd(it, addr) }
                ?: JSONObject().put(
                    ...
                    JSONObject().put(
                        "code",
                        "LIVE_SESSION_REQUIRED"
                    ).put("message", "breakpoints require a live emulator session")
                )
            wrapUnidbgResult(
                result.put(
                    "emulatorSessionId",
        ...
        "session_debugger_status" -> {
            ...
            wrapUnidbgResult(
                (
                    session.live?.let { unidbg.sessionDebuggerStatus(it) }
                        ?: JSONObject().put(
                            "ok",
                            false
                        ).put("error", JSONObject().put("code", "LIVE_SESSION_REQUIRED"))
        ...
        "session_breakpoint_remove" -> {
            ...
            wrapUnidbgResult(
                (
                    session.live?.let { unidbg.sessionBreakpointRemove(it, addr) }
                        ?: JSONObject().put(
                            "ok",
                            false
                        ).put("error", JSONObject().put("code", "LIVE_SESSION_REQUIRED"))
        ...
        "session_single_step" -> {
            ...
            wrapUnidbgResult(
                (
                    session.live?.let { unidbg.sessionSingleStep(it, args.optInt(1, 1)) }
                        ?: JSONObject().put(
                            "ok",
                            false
                        ).put("error", JSONObject().put("code", "LIVE_SESSION_REQUIRED"))
        ...
        "session_emu_stop" -> {
            ...
            wrapUnidbgResult(
                (
                    session.live?.let { unidbg.sessionEmuStop(it) }
                        ?: JSONObject().put(
                            "ok",
                            false
                        ).put("error", JSONObject().put("code", "LIVE_SESSION_REQUIRED"))
        ...
        "session_memory_write" -> {
            ...
            val result =
                session.live?.let { unidbg.sessionMemoryWrite(it, addr, args.optString(2)) }
                    ?: JSONObject().put(
                        "ok",
                        false
                    ).put(
                        ...
                        JSONObject().put(
                            "code",
                            "LIVE_SESSION_REQUIRED"
                        ).put("message", "memory write requires a live emulator session")
                    )
            wrapUnidbgResult(
                result.put(
                    "emulatorSessionId",
        ...
        "session_memory_map" -> {
            ...
            val result =
                session.live?.let { unidbg.sessionMemoryMap(it, addr, size, args.optInt(3, 3)) }
                    ?: JSONObject().put(
                        "ok",
                        false
                    ).put(
                        ...
                        JSONObject().put(
                            "code",
                            "LIVE_SESSION_REQUIRED"
                        ).put("message", "memory map requires a live emulator session")
                    )
            wrapUnidbgResult(
                result.put(
                    "emulatorSessionId",
        ...
        "session_memory_protect" -> {
            ...
            val size = (args.opt(2) as? Number)?.toLong() ?: parseHexLong(args.optString(2)) ?: 0L
            val result =
                session.live?.let { unidbg.sessionMemoryProtect(it, addr, size, args.optInt(3, 3)) }
                    ?: JSONObject().put(
                        "ok",
                        false
                    ).put(
                        ...
                        JSONObject().put(
                            "code",
                            "LIVE_SESSION_REQUIRED"
                        ).put("message", "memory protect requires a live emulator session")
                    )
            wrapUnidbgResult(
                result.put(
                    "emulatorSessionId",
            ...
            val result = session.live?.let { unidbg.sessionMemoryUnmap(it, addr, size) }
                ?: JSONObject().put(
                    ...
                    JSONObject().put(
                        "code",
                        "LIVE_SESSION_REQUIRED"
                    ).put("message", "memory unmap requires a live emulator session")
                )
            wrapUnidbgResult(
                result.put(
                    "emulatorSessionId",
            ...
            val result = session.live?.let { unidbg.sessionReflectRoots(it) }
                ?: JSONObject().put(
                    ...
                    JSONObject().put(
                        "code",
                        "LIVE_SESSION_REQUIRED"
                    ).put("message", "reflection requires a live emulator session")
                )
            wrapUnidbgResult(
                result.put(
                    "emulatorSessionId",
        ...
        "reflect_invoke" -> {
            ...
            val result =
                session.live?.let {
                    unidbg.sessionReflectInvoke(
                        it,
                        args.optString(1),
                        method,
                        args.optJSONArray(2) ?: JSONArray()
                    )
                }
                    ?: JSONObject().put(
                        "ok",
        ...
        "native_schemas" -> {
            ...
            wrapUnidbgResult(
                result.put("emulatorSessionId", session.id).put("workspaceId", session.workspaceId)
            )
        }
        ...
        "native_tool" -> {
            ...
            if (toolName.isBlank()) {
                return@guarded err(
                    "INVALID_ARGUMENT",
                    "native_tool requires a tool name",
                    "method",
                    method
                )
            }
        ...
        "call" -> emulate(
            workspaceId,
            editSessionId,
            method.ifBlank { args.optString(0) },
            args.optJSONArray(1) ?: JSONArray(),
        ...
        "dump" -> dumpMemory(
            workspaceId,
            editSessionId,
            parseHexLong(args.optString(0)) ?: 0L,
            args.optInt(1, 256)
        )

        "reflect" -> err(
            "INVALID_ARGUMENT",
            "Use reflect_roots, reflect_methods, or reflect_invoke with a live emulatorSessionId",
            "op",
            op
        )
...
private fun EngineRuntime.unidbgFrameworkMatrix(): JSONObject = JSONObject()
        ...
        JSONArray(
            listOf(
                JSONObject().put(
                    ...
                    "items",
                    JSONArray(
                        listOf("createDalvikVM", "JNIEnv", "JNI_OnLoad", "Java_* JNI exported call")
                    )
                ),
                JSONObject().put(
                    "area",
    ...
    .put(
        ...
        JSONArray(
            listOf(
                "Run emulate_call with trace=false",
                "If it fails, retry trace=true on a small symbol",
                "Extract missing class/method/syscall from error.stage or trace",
                "Generate stub_template or hook_template",
                "Implement targeted hook in UnidbgEmulator",
                "Retry emulate_call"
            )
        )
    )
...
private fun EngineRuntime.unidbgDebuggerPlan(op: String): JSONObject = JSONObject()
    ...
    .put(
        "reason",
        "Persistent live emulator sessions are enabled for session_call/session_dump. This specific debugger operation still needs a typed MCP wrapper and guardrails before being exposed."
    )
    .put(
        "implementationPlan",
        JSONArray(
    ...
    .put(
        "safeAlternativesNow",
        JSONArray(
            listOf("session_call", "session_dump", "modules", "exports", "imports", "framework_matrix", "hook_template", "stub_template", "env_template")
        )
    )
...
internal fun EngineRuntime.emulationStatus(): JSONObject {
    ...
    return JSONObject()
        ...
        .put(
            "scope",
            "Full Unidbg Android native emulation path backed by Unicorn2 native backend: SO load, DalvikVM/JNIEnv, JNI_OnLoad, exported symbol calls, Java_* JNI calls, and post-load memory dump"
        )
        .put(
            "setup",
            when {
        ...
        .put(
            ...
            JSONObject()
                ...
                .put(
                    ...
                    JSONArray(
                        listOf(
                            ...
                            "AndroidResolver",
                            "DalvikVM creation",
                            "JNIEnv for Java_* calls",
                            "JNI_OnLoad pre-call",
                            "exported symbol call",
                            "post-load memory dump"
                        )
                    )
                )
                .put(
                ...
                .put(
                    "truth",
                    "This uses the real Unidbg and Unicorn2 execution path. App-specific Android framework behavior is resolved by targeted hooks/stubs when a concrete SO demands it."
                )
        )
        .put(
            "coverageModel",
            JSONObject()
                ...
                .put(
                    "nativeLoadError",
                    UnidbgEmulator.nativeDependencyError()?.toString() ?: JSONObject.NULL
                )
                .put(
                    "availabilityError",
        ...
        .put(
            ...
            JSONArray()
                .put(
                    "Android framework classes, syscalls, filesystem, and anti-analysis behavior are handled by concrete Unidbg hooks/stubs when a target SO requires them"
                )
                .put(
                    "Use trace=true for diagnostics, inspect error.stage/error.nextActions, then add the exact missing hook/stub before retrying"
                )
        )
        .put(
            "nextActions",
            JSONArray(
                listOf(
                    "Start with symbolName=JNI_OnLoad or an exported Java_* symbol",
                    "If Java/framework lookup fails, identify the exact missing class/method/syscall from error.stage or trace",
                    "Implement the specific Unidbg hook/stub and retry the same symbol"
                )
            )
        )
}
...
internal fun EngineRuntime.emulate(workspaceId: String, editSessionId: String, symbolName: String, args: JSONArray, trace: Boolean): JSONObject = guarded {
    ...
    if (result.optString("returnValue") ==
        "-1"
    ) {
        result.put(
            "semanticWarning",
            "Unidbg completed without backend exception, but returnValue=-1 may be target-specific or indicate an unmodeled signal/syscall/framework path. Validate with trace, mapped arguments, and target-specific hooks."
        )
    }
    wrapUnidbgResult(
        result.put(
            "workspaceId",
            workspaceId
        ).put("symbolName", symbolName).put("architecture", elf.architecture)
    )
}
...
private fun EngineRuntime.unidbgAddressSpaceHint(): JSONObject = JSONObject()
    ...
    .put(
        "note",
        "Unidbg memory APIs use runtime absolute virtual addresses. For ELF RVA/VA, add the module base returned by unidbg_session(action=modules) or session_modules."
    )
...
private fun EngineRuntime.wrapUnidbgResult(result: JSONObject): JSONObject {
    ...
    val error =
        result.optJSONObject("error")
            ?: JSONObject().put(
                "code",
                "EMULATION_ERROR"
            ).put("message", "Unidbg returned an unsuccessful result")
    ...
    val wrapped = err(
        ...
        message,
        "unidbg",
        result.optString("stage", "unknown"),
        "unidbg" to result
    )
