...
internal fun EngineRuntime.rzFunctions(
    ...
    limit: Int = SettingsStore(context).defaultLimit,
    cursor: String = ""
): JSONObject = guarded {
    val bytes = dataFor(workspaceId, editSessionId)
    val elf = elfFor(workspaceId, editSessionId)
    ...
    if (arr != null && arr.length() > 0) {
        ...
        return@guarded page(
            ...
            "workspaceId",
            workspaceId
        ).put("architecture", elf.architecture).put("source", "rizin")
    }
    val wsName = workspaces[workspaceId]?.source?.name ?: "lib.so"
    val fallback = (elf.symbols + elf.dynSymbols)
        ...
        .map {
            JSONObject().put(
                "locator",
                "so_function:$wsName!${it.name}@${hex(it.value and -2L)}"
            ).put("name", it.name).put(
                "offset",
                it.value and -2L
            ).put(
                "addr",
                ...
                "calls",
                0
            ).put("kind", functionKind(elf, it)).put("source", "lief-dynsym-fallback")
        }
    val warning = if (arr ==
        null
    ) {
        "Rizin function JSON could not be parsed; using LIEF exported FUNC symbols as fallback."
    } else {
        "Rizin returned no functions; using LIEF exported FUNC symbols as fallback."
    }
    page(
        "functions",
        fallback,
...
internal fun EngineRuntime.rzCfg(workspaceId: String, editSessionId: String, locator: String): JSONObject = guarded {
    ...
    val payload = JSONObject(
        result
    ).put("workspaceId", workspaceId).put("functionName", name).put("functionVa", hex(funcVa))
    normalizeCfgPayload(payload, elf)
    if (payload.has("error")) {
        return@guarded err(
            "RIZIN_CFG_FAILED",
            payload.optString("error", "Rizin CFG failed"),
            "locator",
            locator,
            "payload" to payload
        )
...
internal fun EngineRuntime.rzXrefs(workspaceId: String, editSessionId: String, locator: String, direction: String = "to"): JSONObject = guarded {
    ...
    val name = LocatorParser.normalizeSymbol(
        LocatorParser.target(
            locator,
            if (locator.startsWith("so_function:")) "so_function" else "so_symbol"
        )
    )
    ...
    val atVa = LocatorParser.address(locator)
        ...
        ?: return@guarded err(
            "SYMBOL_NOT_FOUND",
            "Symbol not found and locator has no address or PLT fallback",
            "locator",
            locator
        )
    ...
    if (direction == "to" || direction == "both") {
            ...
            if (seen.add(key)) {
                xrefs.put(
                    JSONObject()
                        ...
                        .put(
                            "evidence",
                            "${relocation.section} relocation maps ${hex(
                                relocation.offset
                            )} to $bareName"
                        )
                )
        ...
        if (elf.architecture == "arm64") {
            ...
            aarch64ComputedXrefs(bytes, elf, targets).forEach { item ->
                val key = "${item.optLong(
                    "from"
                )}:${item.optLong("to")}:${item.optString("sourceType")}:incoming"
        ...
        while (offset + 8 <= end) {
            ...
            if (adrp and 0x9f000000.toInt() == 0x90000000.toInt()) {
                ...
                for (lookAhead in 1..4) {
                    ...
                    val scale = if (addImmediate) if (next and 0x00400000 !=
                        0
                    ) {
                        4096L
                    } else {
                    ...
                    found += JSONObject()
                        ...
                        .put(
                            "evidence",
                            "AArch64 ADRP + ${if (loadUnsigned) "LDR" else "ADD"} resolves to ${hex(
                                target
                            )}"
                        )
                }
            ...
            if (insn and 0xFFFFFC1F.toInt() == 0xD63F0000.toInt()) {
                ...
                if (hit != null) {
                    found += JSONObject()
                        ...
                        .put(
                            "evidence",
                            "AArch64 LDR GOT ${hex(hit.first)} + BLR X$rn for $symbolName"
                        )
                }
            }
...
internal fun EngineRuntime.rzSearchBytes(workspaceId: String, editSessionId: String, pattern: String, fromVa: Long = 0, toVa: Long = 0): JSONObject = guarded {
    ...
    val payload = JSONObject(
        result
    ).put("workspaceId", workspaceId).put("pattern", pattern).put("architecture", elf.architecture)
    ...
    if (payload.has("error") &&
        ...
        return@guarded err(
            "RIZIN_SEARCH_FAILED",
            payload.optString("error", "Rizin byte search failed"),
            "pattern",
            pattern,
            "payload" to payload
        )
...
internal fun EngineRuntime.rzEsilStep(workspaceId: String, editSessionId: String, locator: String, stepCount: Int = 1): JSONObject = guarded {
    ...
    val result = NativeEngine.active().esilStep(
        bytes,
        elf.architecture,
        startVa,
        stepCount.coerceIn(1, 1000)
    )
    ok(
        JSONObject(
            result
            ...
            "workspaceId",
            workspaceId
        ).put("functionName", name).put("startVa", hex(startVa)).put("stepCount", stepCount)
    )
}
...
internal fun EngineRuntime.rzDiff(workspaceIdA: String, editSessionIdA: String, workspaceIdB: String, editSessionIdB: String): JSONObject = guarded {
    ...
    ok(
        JSONObject(
            ...
            "workspaceIdA",
            workspaceIdA
        ).put("workspaceIdB", workspaceIdB).put("diffBackend", "rizin")
    )
}
...
internal fun EngineRuntime.rzCommand(workspaceId: String, editSessionId: String = "", command: String, unsafe: Boolean = false): JSONObject = guarded {
    ...
    val payload = JSONObject(
        ...
        "architecture",
        elf.architecture
    ).put("unsafe", unsafe).put("mode", if (unsafe) "rizin_raw_full" else "rizin_raw_readonly")
    if (payload.has("error")) {
        return@guarded err(
            "RIZIN_COMMAND_FAILED",
            payload.optString("error", "Rizin command failed"),
            "command",
            command,
            "payload" to payload
        )
    ...
    if (unsafe && payload.optBoolean("mutated", false)) {
        ...
        for (i in 0 until patches.length()) {
            ...
            val patchBytes =
                hexToBytes(patch.getString("hex"))
                    ?: return@guarded err(
                        "RIZIN_COMMAND_FAILED",
                        "Native Rizin returned invalid patch bytes"
                    )
            if (offset < 0 ||
                offset + patchBytes.size > patched.size
            ) {
                return@guarded err(
                    "RIZIN_COMMAND_FAILED",
                    "Native Rizin patch range exceeds workspace bytes"
                )
            }
        ...
        payload.put(
            ...
            "sha256Before",
            sha256(bytes)
        ).put("sha256After", sha256(patched)).put("patchCount", patches.length())
    }
    ok(payload)
}
...
private fun EngineRuntime.parseAxtCommand(command: String): Pair<Long, Boolean>? {
    ...
    Regex(
        """(?i)axtj?\s*@\s*(0x[0-9a-fA-F]+|[0-9a-fA-F]+)"""
    ).find(cmd)?.groupValues?.getOrNull(1)?.let {
        LocatorParser.hex(it)
    }?.let {
        return it to asJson
    }
    // s 0x1c6f0; axtj  or  s 0x1c6f0\naxt
    Regex(
        """(?i)s\s+(0x[0-9a-fA-F]+|[0-9a-fA-F]+)\s*[;\n\r]+\s*axtj?"""
    ).find(cmd)?.groupValues?.getOrNull(1)?.let {
        LocatorParser.hex(it)
    }?.let {
        return it to asJson
...
internal fun EngineRuntime.rzDecompile(workspaceId: String, editSessionId: String = "", locator: String = "", strict: Boolean = true): JSONObject = guarded {
    ...
    val va =
        resolveCodeAddress(bytes, elf, target)
            ?: return@guarded err(
                "INVALID_ARGUMENT",
                "locator must resolve to a function or hex VA",
                "locator",
                locator
            )
...
private fun EngineRuntime.fallbackByteSearch(bytes: ByteArray, elf: ElfFile, pattern: String, fromVa: Long, toVa: Long): JSONArray {
    ...
    val start = if (fromVa >
        ...
        vaToOffset(elf, fromVa)?.toInt() ?: fromVa.toInt().coerceIn(0, bytes.size)
    } else {
        0
    }
    val end = if (toVa >
        ...
        vaToOffset(elf, toVa)?.toInt() ?: toVa.toInt().coerceIn(0, bytes.size)
    } else {
        bytes.size
    }
    ...
    while (pos <= boundedEnd - parsed.size && hits.length() < maxHits) {
        if (parsed.indices.all { parsed[it] == null || bytes[pos + it] == parsed[it] }) {
            ...
            hits.put(
                JSONObject().put("fileOffset", hex(pos.toLong())).put(
                    "va",
                    va?.let(::hex) ?: JSONObject.NULL
                ).put(
                    "section",
                    sectionForOffset(elf, pos.toLong())?.name ?: ""
                ).put("length", parsed.size)
            )
...
private fun EngineRuntime.parseHexPattern(pattern: String): List<Byte?>? {
    ...
    if (tokens.size == 1 && !tokens[0].contains('?')) {
        ...
        if (compact.length % 2 != 0 ||
            !compact.all { it.isDigit() || it in 'a'..'f' || it in 'A'..'F' }
        ) {
            return null
        }
    ...
    return tokens.map { token ->
        if (token == "??" ||
            ...
            token.toIntOrNull(16)?.takeIf { it in 0..255 }?.toByte()
                ?: return null
        }
    }
}
    ...
    if (signalNames.isNotEmpty()) {
        payload.put(
            ...
            JSONObject()
                ...
                .put(
                    "staticCfgLimitation",
                    "Signal handlers and asynchronous signal delivery are semantic/runtime edges; Rizin basic-block CFG does not automatically add those edges without concrete handler recovery."
                )
        )
    }
}
...
private fun EngineRuntime.enrichDecompilePayload(payload: JSONObject, bytes: ByteArray, elf: ElfFile, va: Long, name: String, locator: String): JSONObject {
    ...
    val analyzedSize = payload.optLong("functionSize", 0L).takeIf { it > 0 }
        ...
        ?: (elf.symbols + elf.dynSymbols).firstOrNull {
            it.name == name || (it.value and -2L) == va
        }?.size?.takeIf {
            it >
                0
        }
    ...
    val returnInference = inferReturnType(
        bytes,
        elf,
        va,
        if (analyzedSize >
    ...
    val contentAudit =
        auditPseudocodeBoundary(rawPseudo, elf, va, end, nextBoundary, name, nextName)
    ...
    if (resized && nextBoundary > 0) {
        warnings += if (!nextName.isNullOrBlank()) {
            "Analysis function object was resized to next boundary $nextName @ ${hex(
                nextBoundary
            )} before decompilation; pseudocode is uncut decompiler output"
        } else {
            "Analysis function object was resized to next boundary @ ${hex(
                nextBoundary
            )} before decompilation; pseudocode is uncut decompiler output"
        }
    }
    ...
    return payload
        ...
        .put(
            ...
            JSONObject()
                ...
                .put(
                    ...
                    payload.optString(
                        "boundaryStrategy",
                        if (payload.optLong("functionSize", 0L) >
                            0
                        ) {
                            "rizin"
                        } else {
        ...
        .put(
            "pseudocodeCoverage",
            contentAudit.optJSONObject("coverage") ?: JSONObject()
        ...
        .put(
            "pseudocodePolicy",
            "never-hard-clip; root-cause analysis-boundary repair + post content-crossing detection only"
        )
}
...
private fun EngineRuntime.auditPseudocodeBoundary(
    ...
    if (picOutside.isNotEmpty()) {
        ...
        warnings.put(
            "Decompiler PIC/WARNING sites after boundary: ${picOutside.take(4).joinToString {
                hex(it)
            }}"
        )
    }
...
internal fun EngineRuntime.rizinFunctions(bytes: ByteArray, elf: ElfFile): List<JSONObject> {
    ...
    val arr =
        runCatching {
            JSONArray(NativeEngine.active().functions(bytes, elf.architecture))
        }.getOrNull()
            ?: return emptyList()
    return (0 until arr.length()).mapNotNull { arr.optJSONObject(it) }
}
internal fun EngineRuntime.decompileWithContext(
    workspaceId: String,
    editSessionId: String,
    elf: ElfFile,
    va: Long,
