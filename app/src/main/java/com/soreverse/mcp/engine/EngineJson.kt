...
internal object EngineJson {
    fun symbolJson(file: String, s: SymbolInfo) = JSONObject().put(
        "locator",
        "so_symbol:$file!${s.name}"
    ).put(
        "name",
        s.name
    ).put(
        ...
        "isExported",
        s.exported
    ).put("isImported", s.imported).put("demangled", JSONObject.NULL)
    fun stringJson(file: String, s: StringInfo) = JSONObject().put(
        "locator",
        "so_string:$file!${s.offset.toString(16)}"
    ).put(
        "offset",
        hex(s.offset)
    ).put(
        ...
        "section",
        s.section
    ).put("encoding", s.encoding).put("confidence", s.confidence).put("isTerminated", true)
    fun sectionJson(file: String, s: SectionInfo, index: Int = -1) = JSONObject().put(
        "locator",
        sectionLocator(file, s, index)
    ).put(
        "name",
        s.name
    ).put(
        ...
        "link",
        s.link
    ).put("info", s.info).put("addralign", s.addralign).put("entsize", s.entsize)
    fun relocJson(r: RelocInfo) = JSONObject().put(
        "section",
        r.section
    ).put(
        "offset",
        ...
        "typeName",
        relocationTypeName(r.type)
    ).put(
        "symbol",
        r.symbol
    ).put(
        "symbolLocator",
        if (r.symbol.isNotBlank()) "so_symbol:lib.so!${r.symbol}" else JSONObject.NULL
    ).put("addend", r.addend)

    fun relocationJson(file: String, elf: ElfFile, r: RelocInfo): JSONObject {
        val symbol = r.symbol.takeIf { it.isNotBlank() }
        val symbolInfo = symbol?.let { name ->
    ...
    fun phJson(p: ProgramHeaderInfo) = JSONObject().put(
        "type",
        p.type
    ).put(
        "flags",
        ...
        "paddr",
        hex(p.paddr)
    ).put("filesz", p.filesz).put("memsz", p.memsz).put("align", p.align)
    fun dynJson(d: DynamicEntryInfo) = JSONObject().put("tag", d.tag).put("value", hex(d.value))
    fun elfSummaryJson(workspaceId: String, editSessionId: String, elf: ElfFile) = JSONObject().put(
        "workspaceId",
        workspaceId
    ).put(
        "editSessionId",
