...
internal object FlutterArtifactInspector {
    ...
    fun inspectApk(bytes: ByteArray, path: String, requestedAbi: String = "auto"): JSONObject {
        require(bytes.size.toLong() <= MAX_APK_BYTES) {
            "INPUT_LIMIT_EXCEEDED: APK exceeds 512 MiB"
        }
        ...
        ZipInputStream(bytes.inputStream()).use { zip ->
            while (true) {
                ...
                entries++
                require(entries <= MAX_ZIP_ENTRIES) {
                    "INPUT_LIMIT_EXCEEDED: APK contains too many entries"
                }
                val name = entry.name
                if (name.startsWith("assets/flutter_assets/")) assets.put(name)
                if (!entry.isDirectory &&
                    name.matches(Regex("^lib/[^/]+/(libapp|libflutter)\\.so$"))
                ) {
                    val parts = name.split('/')
                    val abi = parts[1]
                    val item = candidates.getOrPut(abi) { linkedMapOf() }
                    item[if (name.endsWith("libapp.so")) "libappEntry" else "libflutterEntry"] =
                        name
                    item[if (name.endsWith("libapp.so")) "libappSize" else "libflutterSize"] =
                        entry.size
                }
                zip.closeEntry()
            }
        ...
        val result = JSONObject()
            ...
            .put(
                "flutter",
                JSONObject().put("detected", selected != null).put(
                    "mode",
                    if (selected !=
                        null
                    ) {
                    ...
                    "supportedAbis",
                    JSONArray(candidates.keys)
                ).put("assets", assets).put("candidates", candidatesJson(candidates))
            )
            ...
            .put(
                ...
                JSONArray(
                    listOf(
                        "Flutter AOT semantic recovery requires a matching Blutter Dart VM runner",
                        "This inspector does not execute or decompile target code"
                    )
                )
            )
        ...
        val selected =
            inventory.optJSONObject("selected")
                ?: throw IllegalArgumentException(
                    "FLUTTER_LIBS_NOT_FOUND: APK does not contain a complete libapp.so/libflutter.so pair"
                )
    ...
    fun inspectLibraries(libraries: FlutterLibraries): JSONObject {
        require(libraries.libapp.size.toLong() <= MAX_SO_BYTES) {
            "INPUT_LIMIT_EXCEEDED: libapp.so exceeds 256 MiB"
        }
        require(libraries.libflutter.size.toLong() <= MAX_SO_BYTES) {
            "INPUT_LIMIT_EXCEEDED: libflutter.so exceeds 256 MiB"
        }
        ...
        val fingerprint = FlutterFingerprint(
            ...
            architecture = if (app.machine ==
                183
            ) {
                "arm64"
            } else if (app.machine == 62) {
                "x64"
            } else {
                null
            },
    ...
    private fun selectAbi(
        ...
        return order.asSequence().mapNotNull { abi ->
            candidates[abi]?.takeIf {
                it.containsKey("libappEntry") &&
                    it.containsKey("libflutterEntry")
            }?.let { AbiCandidate(abi, it) }
        }.firstOrNull()
    }
    ...
    private fun parseElf(bytes: ByteArray, label: String): ElfInfo {
        require(
            ...
            "APK_INVALID: $label is not an ELF file"
        }
        ...
        val sectionOffset = if (klass ==
            2
        ) {
            u64(bytes, 0x28, order)
        } else {
            u32(bytes, 0x20, order).toLong()
        }
        ...
        sections.forEachIndexed { index, section ->
            ...
            if (section.type ==
                ...
                symbols += parseSymbols(bytes, section, sections, klass, order, loads)
            }
        }
    ...
    private fun readSections(
        ...
        if (klass ==
            ...
            Section(
                u32(bytes, base, order),
                u32(bytes, base + 4, order),
                u64(bytes, base + 24, order),
                u64(
                    bytes,
                    base + 32,
                    order
                ),
                u32(bytes, base + 40, order),
                u64(bytes, base + 56, order)
            )
        } else {
            Section(
                u32(bytes, base, order),
                u32(bytes, base + 4, order),
                u32(bytes, base + 16, order).toLong(),
                u32(
                    bytes,
                    base + 20,
                    order
                ).toLong(),
                u32(bytes, base + 24, order),
                u32(bytes, base + 36, order).toLong()
            )
        }
    }
    ...
    private fun readLoadSegments(
        ...
        return (0 until count).mapNotNull { index ->
            ...
            if (klass ==
                ...
                LoadSegment(
                    u64(bytes, base + 16, order),
                    u64(bytes, base + 8, order),
                    u64(
                        bytes,
                        base + 32,
                        order
    ...
    private fun parseSymbols(
        ...
        val table = slice(bytes, strings.offset, strings.size)
        val count = if (section.entsize >
            0
        ) {
            (section.size / section.entsize).toInt().coerceAtMost(200_000)
        } else {
            0
        }
        return (0 until count).mapNotNull { index ->
            ...
            if (klass == 2 && base + 24 <= bytes.size) {
                ...
                Symbol(
                    readCString(table, u32(bytes, base, order).toInt()),
                    value,
                    u64(
                        bytes,
                        base + 16,
                    ...
                    virtualToFileOffset(value, loads)
                )
    ...
    private fun snapshotEvidence(bytes: ByteArray, symbols: List<Symbol>): SnapshotEvidence {
        val symbol =
            symbols.firstOrNull { it.name == "_kDartVmSnapshotData" }
                ?: return SnapshotEvidence(
                    null,
                    emptyList(),
                    listOf(
                        "libapp.so does not expose _kDartVmSnapshotData in its dynamic symbol table"
                    )
                )
        ...
        if (start < 0 ||
            ...
            return SnapshotEvidence(
                null,
                emptyList(),
                listOf("_kDartVmSnapshotData is outside the file-backed range")
            )
        }
        val hash = bytes.copyOfRange(start, start + 32).toString(Charsets.US_ASCII).takeIf {
            it.matches(Regex("[ -~]{32}"))
        }
        val flagsBytes = bytes.copyOfRange(start + 32, minOf(bytes.size, start + 288))
        val flags = flagsBytes.toString(
            Charsets.US_ASCII
        ).substringBefore('\u0000').trim().split(Regex("\\s+")).filter {
            it.isNotBlank()
        }
        return SnapshotEvidence(
            hash,
            ...
            listOf(
                ...
                "snapshot hash and flags read using Blutter extract_dart_info compatibility rules"
            )
        )
    }
    ...
    private fun engineEvidence(bytes: ByteArray, rodata: ByteArray): EngineEvidence {
        ...
        val ids = Regex("(?<![a-f0-9])([a-f0-9]{40})(?![a-f0-9])").findAll(text).map {
            it.groupValues[1]
        }.distinct().toList()
        ...
        return EngineEvidence(
            ...
            version,
            listOf(
                "libflutter.so .rodata",
                "engine IDs and Dart channel/version searched using Blutter compatibility rules"
            )
        )
    }
    ...
    private fun slice(bytes: ByteArray, offset: Long, size: Long): ByteArray = if (offset >= 0 &&
        size >= 0 &&
        offset <= bytes.size &&
