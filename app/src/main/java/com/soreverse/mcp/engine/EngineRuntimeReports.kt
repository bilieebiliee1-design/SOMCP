...
internal fun EngineRuntime.analysisReport(workspaceId: String, editSessionId: String = "", writeToFile: Boolean = true): JSONObject = guarded {
    ...
    val sections =
        list(resolvedId, editSessionId, "sections", "", 5000).optJSONArray("items") ?: JSONArray()
    val dynsyms =
        list(resolvedId, editSessionId, "dynsyms", "", 5000).optJSONArray("items") ?: JSONArray()
    val strings =
        strings(resolvedId, editSessionId, "", "", 500, "", "").optJSONArray("items") ?: JSONArray()
    ...
    if (elf.sections.isEmpty()) {
        recommendations.put(
            "No section headers detected: run edit_fix_sections before section-based patching"
        )
    }
    if (functions.length() ==
        0
        ...
        recommendations.put(
            "No Rizin functions detected: use read_disasm(addr=<.text.virtualAddr>) and analyze_elf dynsyms/sections"
        )
    }
    if ((crypto.optJSONArray("items")?.length() ?: 0) >
        0
    ) {
        recommendations.put(
            "Crypto-like constants or high-entropy regions detected: inspect analyze_crypto findings"
        )
    }
    if ((elf.dynSymbols.count { it.name == "JNI_OnLoad" || it.name.startsWith("Java_") }) >
        0
    ) {
        recommendations.put(
            "JNI exports detected: use emulate_call for JNI_OnLoad or Java_* validation"
        )
    }
    val payload = JSONObject()
        ...
        .put(
            "imports",
            list(resolvedId, editSessionId, "imports", "", 5000).optJSONArray("items")
                ?: JSONArray()
        )
        .put(
            "relocations",
            list(resolvedId, editSessionId, "relocations", "", 5000).optJSONArray("items")
                ?: JSONArray()
        )
        .put("strings", strings)
        ...
        .put(
            "security",
            analyze(resolvedId, editSessionId).optJSONObject("security") ?: JSONObject()
        )
        .put("recommendations", recommendations)
    val file = if (writeToFile) {
        val dir = reportDir()
        val safeName = ws.source.name.replace(Regex("[^A-Za-z0-9._-]"), "_")
        File(dir, "$safeName.${System.currentTimeMillis()}.analysis-report.json").also {
            it.writeText(payload.toString(2))
        }
    } else {
        null
    }
    ok(
        JSONObject().put("report", payload).put("written", file != null).put(
            "reportPath",
            file?.absolutePath ?: JSONObject.NULL
        )
    )
}
    ...
    if (dir.exists()) {
        dir.listFiles { f ->
            ...
            ?.forEach { f ->
                ...
                items.put(
                    meta.put("file", f.name).put("path", f.absolutePath).put("size", f.length())
                )
            }
    }
    ok(
        JSONObject().put(
            ...
            "directory",
            dir.absolutePath
        ).put("pagination", pagination(false, null, items.length(), bounded, items.length()))
    )
}
...
internal fun EngineRuntime.loadAudit(file: String): JSONObject = guarded {
    ...
    if (!f.exists() ||
        ...
        return@guarded err("AUDIT_NOT_FOUND", "Audit file not found: $file", "file", file)
    }
    val obj = runCatching {
        ...
        return@guarded err(
            "AUDIT_CORRUPTED",
            "Audit file is not valid JSON: ${it.message}",
            "file",
            file
        )
    }
...
internal fun EngineRuntime.diff(
    workspaceId: String,
    ...
    val ws =
        workspaces[workspaceId] ?: return@guarded err("WORKSPACE_NOT_FOUND", "Workspace not found")
    val session =
        ws.edits[editSessionId]
            ?: return@guarded err("EDIT_SESSION_NOT_FOUND", "Edit session not found")
    val baselineWs = if (compareWorkspaceId.isNotBlank()) {
        workspaces[compareWorkspaceId]
            ?: return@guarded err("WORKSPACE_NOT_FOUND", "Compare workspace not found")
    } else {
        ws
    }
    val compareData = if (compareSessionId.isBlank()) {
        ws.data
    } else {
        baselineWs.edits[compareSessionId]?.data
            ?: return@guarded err(
                "EDIT_SESSION_NOT_FOUND",
                "Compare edit session not found: $compareSessionId"
            )
    }
    ...
    while (i < maxSize && ranges.length() < cap) {
        ...
        val start = i
        while (i < maxSize &&
            !(i < compareData.size && i < session.data.size && compareData[i] == session.data[i])
        ) {
            i++
        }
        val old = if (start <
            ...
            compareData.copyOfRange(start, min(i, compareData.size))
        } else {
            ByteArray(0)
        }
        val next = if (start <
            ...
            session.data.copyOfRange(start, min(i, session.data.size))
        } else {
            ByteArray(0)
        }
        ranges.put(
            ...
            ).put("newHex", PatchByteUtils.hexBytes(next))
        )
    }
...
internal fun EngineRuntime.writePatchReport(workspaceId: String, session: EditSession, output: File): File {
    ...
    val payload = JSONObject()
        ...
        .put(
            "snapshots",
            session.snapshots.mapIndexed { idx, s ->
                JSONObject().put(
                    "index",
                    idx
                ).put(
