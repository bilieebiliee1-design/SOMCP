...
internal fun EngineRuntime.capabilityRegistry(): JSONObject = JSONObject()
    ...
    .put(
        "truth",
        "Entries are generated from code paths that are callable through MCP and verified against packaged runtime modules. Commercial-only upstream modules are never advertised as native open-source implementations."
    )
    .put(
        "backends",
        JSONObject()
            .put(
                ...
                JSONObject()
                    ...
                    .put(
                        ...
                        JSONObject().put(
                            "unsafeCommands",
                            "require unsafe=true and authenticated MCP access"
                        ).put("mutations", "returned as persistent edit sessions")
                    )
            )
            .put(
                "lief",
                JSONObject()
                    ...
                    .put(
                        "compiledFormats",
                        JSONArray(listOf("ELF", "PE", "Mach-O", "DEX", "ART", "OAT", "VDEX"))
                    )
                    .put(
                        "supported",
                        JSONArray(
                    ...
                    .put(
                        ...
                        JSONArray(
                            listOf(
                                "Rizin assembly",
                                "Rizin debug info",
                                "Rizin Objective-C analysis",
                                "Rizin dyld cache analysis"
                            )
                        )
                    )
            )
            .put(
                ...
                JSONObject()
                    ...
                    .put(
                        "dynamicCoverage",
                        "All public methods reachable from packaged live emulator/vm/module/backend/memory objects are discoverable and invokable through object handles"
                    )
            )
            .put(
                "xanso",
                JSONObject()
                    .put(
                        ...
                        JSONObject().put(
                            "available",
                            xanso.available()
                        ).put("backend", "freakishfox/xAnSo upstream + xAnSo64 extension")
                    )
                    ...
                    .put(
                        ...
                        JSONArray(
                            listOf(
                                "help",
                                "build-section",
                                "ELF32_ARM_upstream_section_fix",
                                "ELF64_LIEF_assisted_reconstruction",
                                "persistent_edit_session",
                                "sha256_and_diff_evidence"
                            )
                        )
                    )
                    .put(
...
internal fun EngineRuntime.liefDispatch(
    ...
    when (op) {
        "roots" -> return@guarded ok(
            JSONObject().put(
                "roots",
                JSONArray(
                    listOf("binary", "sections", "symbols", "dynSymbols", "relocations", "programHeaders", "dynamicEntries", "strings", "nativeJson")
                )
            ).put("formats", JSONArray(listOf("ELF", "PE", "Mach-O", "DEX", "ART", "OAT", "VDEX")))
        )

        "methods" -> return@guarded ok(
            JSONObject().put(
                "methods",
                JSONArray(
                ...
                "settableObjectPaths",
                JSONArray(listOf("sections[i].content", "symbols[i].name", "dynSymbols[i].name"))
            ).put(
                "argumentTemplates",
                JSONObject().put(
                    "parseAny",
                    JSONArray(listOf("format:auto|elf|pe|macho|dex|art|oat|vdex"))
                ).put(
                    "nativeGet",
                    JSONArray(listOf("format", "objectPath"))
                ).put(
                    "setSectionContent",
                    JSONArray(listOf("sectionName", "hexContent"))
                ).put(
                    "patchAddress",
                    JSONArray(listOf("vaHex", "patchHex"))
                ).put(
                    "addExportedFunction",
                    JSONArray(listOf("vaHex", "name"))
                ).put(
                    "removeSymbol",
                    JSONArray(listOf("name"))
                ).put("build", JSONArray(listOf("outputName", "conflictStrategy")))
            )
        )

        "parse_any" -> return@guarded ok(
            JSONObject().put(
                "format",
                args.optString(0, "auto")
            ).put(
                "binary",
                lief.parseAny(dataFor(workspaceId, editSessionId), args.optString(0, "auto"))
            )
        )
        ...
        "native_snapshot" -> {
            ...
            return@guarded ok(
                JSONObject().put(
                    ...
                    "source",
                    "LIEF::to_json"
                ).put("binary", lief.parseAny(dataFor(workspaceId, editSessionId), format))
            )
        }
        ...
        "native_get", "native_list" -> {
            ...
            return@guarded ok(
                JSONObject().put(
                    "format",
                    format
                ).put("source", "LIEF::to_json").put("objectPath", objectPath).put(
                    if (op ==
                        "native_list"
                    ) {
                        "items"
        ...
        "get" -> return@guarded ok(
            JSONObject().put(
                "objectPath",
                objectPath
            ).put("value", resolveLiefObjectPath(elf, objectPath))
        )

        "list" -> return@guarded ok(
            JSONObject().put(
                "objectPath",
                objectPath
            ).put("items", resolveLiefObjectPath(elf, objectPath))
        )
        ...
        "call" -> return@guarded when (method) {
            "toJson", "snapshot" -> ok(
                JSONObject().put(
                    "binary",
                    EngineJson.elfSummaryJson(workspaceId, editSessionId, elf)
                )
            )
            ...
            "getSectionContent" -> {
                ...
                ok(
                    JSONObject().put(
                        ...
                        "hexPreview",
                        PatchByteUtils.hexBytes(content.copyOfRange(0, minOf(content.size, 256)))
                    )
                )
            }
            ...
            "setSectionContent" -> {
                ...
                val content =
                    hexToBytes(args.optString(1))
                        ?: return@guarded err(
                            "INVALID_HEX",
                            "args[1] must be hex section content",
                            "args",
                            args
                        )
                if (dryRun) {
                    return@guarded ok(
                        JSONObject().put(
                            ...
                            "method",
                            method
                        ).put("section", sectionName).put("size", content.size)
                    )
                }
                val original = dataFor(workspaceId, editSessionId)
                if (elf.sections.none {
                        it.name == sectionName
                    }
                ) {
                    return@guarded err(
                ...
                if (patched.contentEquals(
                        original
                    )
                ) {
                    return@guarded err(
                        "LIEF_MUTATION_FAILED",
                        "LIEF setSectionContent returned unchanged bytes",
                        "section",
                        sectionName
                    )
                }
            ...
            "patchAddress" -> if (dryRun) {
                ok(
                    JSONObject().put(
                        ...
                        "method",
                        method
                    ).put("va", args.optString(0)).put("patchHex", args.optString(1))
                )
            } else {
                liefPatchAddress(
                    workspaceId,
                    editSessionId,
                    LocatorParser.hex(args.optString(0))
                        ?: return@guarded err(
                            "INVALID_ARGUMENT",
                            "args[0] must be hex VA",
                            "args",
                            args
                        ),
                    hexToBytes(args.optString(1))
            ...
            "addExportedFunction" -> if (dryRun) {
                ok(
                    JSONObject().put(
                        ...
                        "method",
                        method
                    ).put("va", args.optString(0)).put("name", args.optString(1))
                )
            } else {
                liefAddExportedFunction(
                    workspaceId,
                    editSessionId,
                    LocatorParser.hex(args.optString(0))
                        ?: return@guarded err(
            ...
            "removeSymbol" -> if (dryRun) {
                ok(
                    JSONObject().put(
                        "dryRun",
                        true
                    ).put("wouldMutate", true).put("method", method).put("name", args.optString(0))
                )
            } else {
                liefRemoveSymbol(workspaceId, editSessionId, args.optString(0))
            }

            "fixSections" -> if (dryRun) {
                ok(
                    JSONObject().put("dryRun", true).put("wouldMutate", true).put("method", method)
                )
            } else {
                fixSections(workspaceId, editSessionId)
            }
            ...
            "build" -> if (dryRun) {
                ok(
                    JSONObject().put(
                        ...
                        "outputName",
                        args.optString(0, "patched.so")
                    ).put("conflictStrategy", args.optString(1, "rename"))
                )
            } else {
                build(
                    workspaceId,
                ...
                "LIEF method is not implemented by the current dispatcher",
                "method",
                method
            )
        }
        ...
        "set" -> return@guarded when {
            objectPath.startsWith("sections[") && objectPath.endsWith("].content") -> {
                val idx = Regex(
                    "sections\\[(\\d+)]"
                ).find(objectPath)?.groupValues?.getOrNull(1)?.toIntOrNull()
                val sec =
                    idx?.let { elf.sections.getOrNull(it) }
                        ?: return@guarded err(
                            "INVALID_ARGUMENT",
                ...
                val content =
                    hexToBytes(args.optString(0))
                        ?: return@guarded err(
                            "INVALID_HEX",
                            "args[0] must be hex section content",
                            "args",
                            args
                        )
                if (dryRun) {
                    return@guarded ok(
                        JSONObject().put(
                            ...
                            "objectPath",
                            objectPath
                        ).put("section", sec.name).put("size", content.size)
                    )
                }
                val original = dataFor(workspaceId, editSessionId)
                ...
                if (patched.contentEquals(
                    ...
                    return@guarded err(
                        "LIEF_MUTATION_FAILED",
                        "LIEF section mutation returned unchanged bytes",
                        "objectPath",
                        objectPath
                    )
                }
            ...
            objectPath.matches(Regex("(symbols|dynSymbols)\\[\\d+].name")) -> {
                ...
                val sym =
                    idx?.let {
                        if (group ==
                            ...
                            elf.symbols.getOrNull(it)
                        } else {
                            elf.dynSymbols.getOrNull(it)
                        }
                    }
                        ?: return@guarded err(
                            "INVALID_ARGUMENT",
                ...
                if (newName.isBlank()) {
                    return@guarded err(
                        "INVALID_ARGUMENT",
                        "args[0] must be the new symbol name",
                        "args",
                        args
                    )
                }
                if (newName.length >
                    ...
                    return@guarded err(
                        "SYMTAB_OVERFLOW",
                        "Renaming to a longer symbol name is not supported by byte-level string-table replacement",
                        "args",
                        args
                    )
                }
                ...
                if (matches.isEmpty()) {
                    return@guarded err(
                        "SYMBOL_NOT_FOUND",
                        "Symbol string bytes were not found",
                        "objectPath",
                        objectPath
                    )
                }
                if (matches.size !=
                    ...
                    return@guarded err(
                        "AMBIGUOUS_SYMBOL_STRING",
                        "Symbol string bytes matched multiple file offsets; use removeSymbol/addExportedFunction or a section-specific edit instead",
                        "objectPath",
                        objectPath
                    )
                }
                ...
                if (dryRun) {
                    return@guarded ok(
                        JSONObject().put(
                            ...
                            "oldName",
                            sym.name
                        ).put("newName", newName).put("fileOffset", hex(pos.toLong()))
                    )
                }
                ...
                "CAPABILITY_NOT_IMPLEMENTED",
                "Generic LIEF property mutation is not implemented for this objectPath",
                "objectPath",
                objectPath
            )
        }
...
private fun EngineRuntime.resolveJsonObjectPath(root: Any, objectPath: String): Any {
    ...
    for (part in objectPath.split('.').filter { it.isNotBlank() }) {
        ...
        val indexes = Regex("\\[(\\d+)]").findAll(part).mapNotNull {
...
private fun EngineRuntime.validateLiefDispatch(
    ...
    when (method) {
        "setSectionContent" -> {
            ...
            if (hexToBytes(args.optString(1)) ==
                null
            ) {
                issues.put("args[1] must be valid hex content")
            }
        }
        ...
        "patchAddress" -> {
            ...
            if (va ==
                ...
                issues.put("args[0] must be a hex VA")
            } else if (vaToOffset(elf, va) ==
                null
            ) {
                issues.put("args[0] VA does not map to a file offset")
            }
            if (hexToBytes(args.optString(1)) ==
                null
            ) {
                issues.put("args[1] must be valid hex patch bytes")
            }
        }
        ...
        "removeSymbol" -> if (args.optString(
                0
            ).isBlank()
        ) {
            issues.put("args[0] symbol name is required")
        }
    }
    return ok(
        JSONObject().put(
            "valid",
            issues.length() == 0
        ).put("issues", issues).put("method", method).put("objectPath", objectPath)
    )
}
    ...
    return@guarded when (op) {
        "status", "roots", "capabilities" -> ok(
            JSONObject()
                ...
                .put(
                    ...
                    JSONArray(
                        listOf(
                            "ELF32 little-endian ET_DYN ARM via upstream section_fix",
                            "ELF64 via xAnSo64 LIEF-assisted reconstruction"
                        )
                    )
                )
                .put(
                    "scope",
                    "Complete upstream CLI/core public functionality plus ELF64 reconstruction extension"
                )
        )
        ...
        "methods", "help" -> ok(
            JSONObject()
                .put(
                    "methods",
                    JSONArray(listOf("status", "capabilities", "help", "build-section"))
                )
                .put("backend", "freakishfox/xAnSo upstream Core/fix")
...
internal fun EngineRuntime.xansoBuildSections(
    ...
    if (original.size <
        ...
        return@guarded err(
            "ELF_CORRUPTED",
            "Input is too small to contain an ELF identification header"
        )
    }
    ...
    val backend = when (elfClass) {
        ...
        else -> return@guarded err(
            "XANSO_UNSUPPORTED_ELF_CLASS",
    ...
    if (elfClass == 1 &&
        ...
        return@guarded err(
            "XANSO_UNAVAILABLE",
            "Real xAnSo native backend is not loaded for this ABI"
        )
    }
    if (elfClass == 2 &&
        !lief.available()
        ...
        return@guarded err(
            "LIEF_UNAVAILABLE",
            "LIEF is required by the xAnSo64 reconstruction extension"
        )
    }
    ...
    if (alreadyStructured && !force) {
        return@guarded ok(
            JSONObject()
                ...
                .put(
                    "message",
                    "Section table is already present and parseable; set force=true to rebuild it"
                )
    ...
    val recovered = if (elfClass == 2 &&
        beforeElf.sections.isEmpty()
    ) {
        xanso.recoverElf64Sections(original)
    } else {
        null
    }
    ...
    if (afterElf.sections.isEmpty()) {
        return@guarded err(
            "XANSO_BUILD_FAILED",
            "$backend produced an ELF without a usable section table"
        )
    }
    ...
    ok(
        JSONObject()
            ...
            .put(
                "recoveryMode",
                when {
                    elfClass == 1 -> "upstream-section-fix"

                    recovered != null &&
                        recovered.size == original.size -> "orphan-section-table-scan+LIEF"

                    recovered != null -> "program-header-section-synthesis+LIEF"

                    else -> "dynamic-metadata+LIEF"
                }
            )
...
internal fun EngineRuntime.liefPatchAddress(
    ...
    if (!lief.available()) {
        return@guarded err(
            "LIEF_UNAVAILABLE",
            "LIEF native backend not loaded for this ABI"
        )
    }
    val original = dataFor(workspaceId, editSessionId)
    ...
    if (patched.contentEquals(
        ...
        return@guarded err(
            "PATCH_FAILED",
            "LIEF patch_address returned unchanged bytes (VA may not map to a loadable segment)"
        )
    }
...
internal fun EngineRuntime.liefAddExportedFunction(
    ...
    if (patched.contentEquals(
        ...
        return@guarded err(
            "LIEF_MUTATION_FAILED",
            "LIEF add_exported_function returned unchanged bytes",
            "name",
            name
        )
    }
    ...
    if (!verified) {
        return@guarded err(
            "LIEF_POSTCONDITION_FAILED",
            "Exported function was not present after LIEF rebuild",
            "name",
            name
        )
    }
...
internal fun EngineRuntime.liefRemoveSymbol(
    ...
    if ((before.symbols + before.dynSymbols).none {
            it.name == name
        }
    ) {
        return@guarded err("SYMBOL_NOT_FOUND", "Symbol '$name' was not found", "name", name)
    }
    ...
    if (patched.contentEquals(
        ...
        return@guarded err(
            "LIEF_MUTATION_FAILED",
            "LIEF remove_symbol returned unchanged bytes",
            "name",
            name
        )
    }
    val verified = lief.parse(patched)
    if ((verified.symbols + verified.dynSymbols).any {
            it.name == name
        }
    ) {
        return@guarded err(
            "LIEF_POSTCONDITION_FAILED",
            "Symbol remained present after LIEF rebuild",
            "name",
            name
        )
    }
