...
internal fun EngineRuntime.build(
    workspaceId: String,
    ...
    val strategy = (conflictStrategy.ifBlank { settings.outputConflictStrategy }).let {
        if (it ==
            "overwrite"
        ) {
            "overwrite"
    ...
    val report = if (session != null &&
        (writeReport ?: settings.writePatchReport)
    ) {
        writePatchReport(workspaceId, session, out)
    } else {
        null
    }
    ...
    ok(
        JSONObject()
            ...
            .put(
                "warnings",
                if (requestedWorkDirCopy &&
                    !workDirCopy.optBoolean("ok")
                ) {
                    JSONArray().put("workDirCopy failed: ${workDirCopy.optString("message")}")
                } else {
                    JSONArray()
                }
            )
            ...
            .put(
                "openHint",
                "Pass outputPath to so_open, or call build_so (action=list) to enumerate built files."
            )
    )
}

internal fun EngineRuntime.buildMany(
    workspaceId: String,
    editSessionId: String,
    outputs: JSONArray,
    conflictStrategy: String = "",
    ...
    if (outputs.length() ==
        ...
        return@guarded err(
            "INVALID_ARGUMENT",
            "outputs array must not be empty",
            "outputs",
            outputs
        )
    }
    val settings = SettingsStore(context)
    if (outputs.length() >
        settings.defaultBuildVariants
    ) {
        return@guarded err(
            "TOO_MANY_VARIANTS",
            "outputs length ${outputs.length()} exceeds defaultBuildVariants ${settings.defaultBuildVariants}",
            "outputs",
            outputs.length()
        )
    }
    ...
    for (i in 0 until outputs.length()) {
        ...
        val useWorkDir =
            entry?.optBoolean("writeToWorkDir", writeToWorkDir ?: settings.buildCopyToWorkDir)
                ?: (writeToWorkDir ?: settings.buildCopyToWorkDir)
        val wantReport =
            entry?.let {
                if (it.has("writePatchReport")) {
                    it.optBoolean("writePatchReport")
                } else {
                    (
                        writeReport
                            ?: settings.writePatchReport
                        )
                }
            }
                ?: (writeReport ?: settings.writePatchReport)
    ...
    ok(
        JSONObject()
            ...
            .put(
                "openHint",
                "Pass any outputPath to so_open, or call build_so (action=list) to enumerate built files."
            )
    )
}
...
internal fun EngineRuntime.fixSections(workspaceId: String, editSessionId: String): JSONObject {
    ...
    if (fixed.contentEquals(
        ...
        return ok(
            JSONObject().put(
                ...
                "message",
                "No section header reconstruction needed"
            ).put("sha256Before", beforeHash).put("sha256After", afterHash).put("diffRangeCount", 0)
        )
    }
    ...
    if (onlyCanonicalization) {
        return ok(
            JSONObject()
                ...
                .put(
                    "message",
                    "Section headers are already present and parseable; skipped LIEF canonicalization rewrite"
                )
    ...
    val targetSession = editSessionId.takeIf {
        it.isNotBlank()
    }?.let { workspaces[workspaceId]?.edits?.get(it) }
    val sessionId = when {
        targetSession != null && targetSession.data.size == fixed.size -> {
            ...
            targetSession.patches +=
                PatchRecord(
                    System.currentTimeMillis(),
                    "fix_sections",
                    "xanso:fix_sections",
                    LocatorParser.hex(firstDiff?.optString("fileOffset") ?: "0x0")?.toInt() ?: 0,
                    firstDiff?.optString("oldHex") ?: "",
                    firstDiff?.optString("newHex") ?: "",
                    "rebuild section headers; diffRangeCount=${diffRanges.length()}"
                )
            editSessionId
        }
    ...
    return ok(
        JSONObject()
            ...
            .put(
                "appliedToRequestedSession",
                sessionId == editSessionId && editSessionId.isNotBlank()
            )
            .put("sha256Before", beforeHash)
...
private fun EngineRuntime.resolveOutputFile(rawName: String, strategy: String): File {
    ...
    val cleanName = rawName.replace('\\', '/').substringAfterLast('/').trim().ifBlank {
        "patched.so"
    }
...
private fun EngineRuntime.workDirCopyResult(requested: Boolean, copy: () -> String?): JSONObject {
    if (!requested) {
        return JSONObject().put(
            "ok",
            false
        ).put("path", JSONObject.NULL).put("message", "not requested")
    }
        ...
        onSuccess = { path ->
            if (path.isNullOrBlank()) {
                JSONObject().put(
                    ...
                    "path",
                    JSONObject.NULL
                ).put("message", "work directory not configured or copy returned empty path")
            } else {
                JSONObject().put("ok", true).put("path", path).put("message", "copied")
            }
        },
        onFailure = { error ->
            AppLog.w("workDir copy failed: ${error.message}")
            JSONObject().put("ok", false).put("path", JSONObject.NULL).put(
                "message",
