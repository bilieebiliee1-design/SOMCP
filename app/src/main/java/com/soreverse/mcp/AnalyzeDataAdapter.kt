                        ...
                        .put(
                            ...
                            JSONArray().apply {
                                message.parts.forEach { part ->
                                    put(
                                        when (part) {
                                            is RikkaPart.Text -> JSONObject().put(
                                                "type",
                                                "text"
                                            ).put("text", part.text)

                                            is RikkaPart.Reasoning -> JSONObject().put(
                                                "type",
                                                "reasoning"
                                            ).put("text", part.text)
...
internal fun restoreDeepReport(state: AnalyzeUiState, snapshot: JSONObject) {
    ...
    state.deepMessages = (0 until messages.length()).mapNotNull { index ->
        ...
        DeepChatMessage(
            ...
            role = runCatching {
                DeepChatRole.valueOf(message.optString("role"))
            }.getOrDefault(DeepChatRole.ASSISTANT),
    ...
    state.deepReport =
        state.deepMessages.lastOrNull { it.role == DeepChatRole.ASSISTANT }?.text.orEmpty()
...
internal fun loadSoSources(
    ...
    val payload = runCatching {
        ...
        return emptyList<SoSourceUi>() to
            if (zh) "扫描失败：${it.message ?: it.javaClass.simpleName}" else "Scan failed: ${it.message ?: it.javaClass.simpleName}"
    }
    if (!payload.optBoolean("ok", true)) {
        return emptyList<SoSourceUi>() to
            payload.optJSONObject(
                "error"
            )?.optString("message", if (zh) "扫描失败" else "Scan failed").orEmpty()
    }
...
internal fun openSoForUi(
    ...
    return try {
        ...
        if (!analyzed.optBoolean("ok", true)) {
            null to
                analyzed.optJSONObject("error")?.optString("message", "Analyze failed").orEmpty()
        } else {
            ...
            val detail = SoDetailUi(
                ...
                architecture = opened.optString(
                    "architecture",
                    overview.optString("architectureCode", "unknown")
                ),
                bits = opened.optInt("bits", overview.optInt("bits", 0)),
                entryPoint = opened.optString(
                    "entryPoint",
                    overview.optString("entryPoint", "0x0")
                ),
                stripped = analyzed.optBoolean(
                    "stripped",
                    overview.optBoolean(
                        "stripped",
                        counts.optInt("symbols", 0) == 0
                    )
                ),
                hasDebugInfo = analyzed.optBoolean(
                    "hasDebugInfo",
                    overview.optBoolean("hasDebugInfo", false)
                ),
                hasJniOnLoad = analyzed.optBoolean(
                    "hasJniOnLoad",
                    overview.optBoolean("hasJniOnLoad", false)
                ),
            ...
            detail to
                if (zh) "已完成 ${detail.name} 的程序基础分析" else "Basic analysis completed for ${detail.name}"
        }
    } finally {
