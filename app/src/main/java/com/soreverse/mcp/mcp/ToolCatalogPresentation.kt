...
object ToolCatalogPresentation {
    ...
    fun categoryDescriptions(zh: Boolean): List<Pair<String, String>> = listOf(
        "workspace" to
            (if (zh) "工作区生命周期：列出、打开、关闭 SO" else "Workspace lifecycle: list, open, close SO files"),
        "analyze" to
            (if (zh) "深度分析：ELF 结构、函数列表、CFG、密码学扫描、交叉引用、ESIL 模拟" else "Deep analysis: ELF structure, functions, CFG, crypto scan, xrefs, ESIL emulation"),
        ...
        "edit" to
            (if (zh) "补丁：字节/汇编/符号 + xAnSo 节区头重建" else "Patch: bytes/asm/symbols + xAnSo section rebuild"),
        "emulate" to
        ...
        "lowlevel" to
            (if (zh) "底层 API 网关：Rizin / LIEF / Unidbg / xAnSo 聚合入口" else "Low-level API gateways: aggregated Rizin / LIEF / Unidbg / xAnSo access"),
    ...
    fun grouped(zh: Boolean, includeApk: List<String> = emptyList()): List<Pair<String, List<Pair<String, String>>>> {
        ...
        ToolCatalog.ALL.forEach { e ->
            groups[e.meta.category]?.add(e.meta.name to (if (zh) e.meta.zh else e.meta.en))
        }
