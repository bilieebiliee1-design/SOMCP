...
object SchemaBuilder {
    ...
    class PropList {
        ...
        infix fun String.str(description: String) {
            properties.put(this, strProp(description))
        }
        fun String.oneOf(description: String, vararg values: String) {
            properties.put(this, enumProp(description, *values))
        }
        infix fun String.int(description: String) {
            properties.put(this, intProp(description))
        }
        infix fun String.num(description: String) {
            properties.put(this, numProp(description))
        }
        infix fun String.bool(description: String) {
            properties.put(this, boolProp(description))
        }
        infix fun String.arr(description: String) {
            properties.put(this, arrProp(description, null))
        }
        infix fun String.arr(description: Pair<String, JSONObject>) {
            properties.put(this, arrProp(description.first, description.second))
        }
    ...
    fun editsAsmSchema(): JSONObject = JSONObject().put("type", "object")
        .put(
            "description",
            "One assembly edit. Defaults to replacing one instruction at instructionIndex=0; set instructionCount/byteLength to cover multiple instruction slots."
        )
        .put(
            "properties",
            JSONObject()
                .put(
                    "writeAsm",
                    strProp("New assembly text (assembler syntax). Alias: newAsm / asm / assembly.")
                )
                ...
                .put(
                    "address",
                    strProp(
                        "Hex virtual address inside the function, e.g. 0x978. Overrides instructionIndex/byteOffset."
                    )
                )
                .put(
                    "instructionIndex",
                    intProp("Index of the instruction to replace (default 0).")
                )
                .put(
                    "instructionCount",
                    intProp(
                        "Number of instruction slots to cover (default 1). Use >1 to fit multi-instruction payloads."
                    )
                )
                .put(
                    "byteOffset",
                    intProp("Byte offset inside the function (alternative to instructionIndex).")
                )
                .put(
                    "byteLength",
                    intProp(
                        "Byte length to overwrite (alternative to instructionCount). Required for insert_* and write_function modes."
                    )
                )
                .put(
                    "mode",
                    strProp(
                        "replace_instructions (default) | nop_out | delete_instructions | insert_before | insert_after | prepend_function | append_function | write_function"
                    )
                )
        )
    ...
    fun editsHexSchema(): JSONObject = JSONObject().put("type", "object")
        .put(
            "description",
            "One hex edit: write bytes at byteOffset inside the section. Hex aliases are accepted; conflicting alias values are rejected."
        )
        .put(
            "properties",
            JSONObject()
                .put(
                    "newHex",
                    strProp(
                        "Hex bytes to write, e.g. 20 00 80 52. Aliases: hex / bytes / data / rawHex / rawValue."
                    )
                )
                ...
                .put(
                    ...
                    JSONObject().put(
                        "description",
                        "Legacy alias: hex string or array of byte values 0..255."
                    )
                )
                .put(
                    "byteOffset",
                    strProp("Byte offset inside the section (not file offset). Alias: offset.")
                )
    ...
    fun editsSymbolSchema(): JSONObject = JSONObject().put("type", "object")
        .put(
            "description",
            "One symbol edit. rename: same-or-shorter rename. add: add exported function at addr. remove: remove symbol by name."
        )
        .put(
            "properties",
            JSONObject()
                ...
                .put(
                    "newName",
                    strProp("New symbol name (for rename: must be <= original length).")
                )
                .put(
                    "addr",
                    strProp("Hex virtual address for the new exported function (add op only).")
                )
            ...
            JSONObject()
                ...
                .put(
                    ...
                    JSONObject().put(
                        "type",
                        "boolean"
                    ).put("description", "Mirror this variant into the work directory.")
                )
                .put(
                    "writePatchReport",
                    JSONObject().put(
                        "type",
                        "boolean"
                    ).put("description", "Write a patch-report JSON sidecar for this variant.")
                )
        )
            ...
            JSONObject()
                ...
                .put(
                    ...
                    JSONObject().put(
                        ...
                        "description",
                        "Arguments object for this tool. May contain \${resultKey.jsonPath} placeholders substituted from prior steps' results."
                    )
                )
                .put(
                    "resultKey",
                    strProp(
                        "Optional short key (e.g. a0, b1) used to reference this step's result JSON in later steps."
                    )
                )
        )
