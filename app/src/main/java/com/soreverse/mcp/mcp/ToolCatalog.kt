...
object ToolCatalog {
    ...
    private val pathArg: JSONObject.() -> String = {
        str("path").ifBlank {
            str("filePath").ifBlank { str("inputPath").ifBlank { str("soPath") } }
        }
    }
    ...
    private val soOpen = EngineToolHandler(
        ToolMeta(
            "so_open",
            "【SO 分析入口】打开 SO 文件并创建工作区（action=list 列出可用 SO）。所有 .so/.ELF 文件操作必须从 so_open 开始，不要使用 mt_apk_* 桥接工具。",
            "【PRIMARY SO ENTRY POINT】Open a SO file and create a workspace. Use action=list to discover available SO files. Use action=open_url to download a http(s) SO into the selected work directory, then open and analyze it. All .so/ELF tasks MUST start from so_open — do NOT use mt_apk_* bridged tools for SO files.",
            "workspace",
            ToolClass.CORE,
            heavy = true
        ) {
            objectSchema(
                props {
                    ...
                    "url" str
                        "http(s) URL pointing directly to a .so/ELF file (action=open_url). A work directory must be selected first."
                    "outputName" str
                        "Optional file name to save the downloaded SO in the work directory"
        ...
        when (a.str("action", "open")) {
            "list" -> e.listAvailableSos(
                a.str("prefix"),
                a.intValue("limit", s.defaultLimit),
                a.str("cursor")
            )
    ...
    private val apkAnalyze = EngineToolHandler(
        ...
        engine.analyzeApk(
            args.str("path").ifBlank {
                args.str("filePath")
            },
            args.intValue("entryLimit", 500)
        )
    }
    ...
    private val flutterBlutter = EngineToolHandler(
        ToolMeta(
            ...
            objectSchema(
                props {
                    "action".oneOf(
                        "inspect | analyze | status | result | cancel | packages | prune",
                        "inspect",
                        "analyze",
                        "status",
                        "result",
        ...
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "editSessionId" str "Edit session ID (optional)"
                    "locator" str
                        "Function locator. Accepts full locator from analyze_functions (so_function:file!Name) or short function name."
                }
            )
        }
        ...
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "editSessionId" str "Edit session ID (optional)"
                    "locator" str
                        "Symbol/function locator. Accepts full locator from analyze_elf/analyze_functions or short symbol name."
    ...
    private val analyzeEsil = EngineToolHandler(
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "editSessionId" str "Edit session ID (optional)"
                    "locator" str
                        "Function locator or hex VA. Accepts full locator from analyze_functions, short function name, or 0x... address."
                    "addr" str
                        "Hex virtual address fallback when no function symbol is available, e.g. 0x1234."
        ...
        e.rzEsilStep(
            a.str("workspaceId"),
            a.str("editSessionId"),
            a.str("locator").ifBlank {
                a.str("addr")
            },
            a.intValue("stepCount", 1)
        )
    }
    ...
    private val searchBytes = EngineToolHandler(
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "editSessionId" str "Edit session ID (optional)"
                    "pattern" str
                        "Rizin byte pattern: compact hex such as 5F2403D5, nibble wildcard using . such as 5F24..D5, optional bytes:mask; spaced hex and ?? are normalized for compatibility"
        ...
        e.rzSearchBytes(
            ...
            a.str("editSessionId"),
            a.str("pattern"),
            HexCodec.long(a.str("fromVa")) ?: 0L,
            HexCodec.long(a.str("toVa")) ?: 0L
        )
    }
    ...
    private val searchStrings = EngineToolHandler(
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "editSessionId" str "Edit session ID (optional)"
                    "prefix" str
                        "String contains filter or regex pattern, supports Chinese/UTF-8 text"
                    ...
                    "minConfidence" num
                        "Minimum string confidence score in [0,1]; useful to drop noisy UTF-16 candidates"
        ...
        e.strings(
            ...
            a.bool("ignoreCase", true), a.str("encoding"), a.doubleValue("minConfidence", 0.0)
        )
    }
    ...
    private val readDisasm = EngineToolHandler(
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "editSessionId" str "Edit session ID (optional)"
                    "locator" str
                        "Function locator. Accepts full locator from analyze_functions or short function name."
                    ...
                    "addr" str
                        "Hex virtual address fallback; ARM32 Thumb may use odd address or thumb=true"
        ...
        e.disasm(
            ...
            a.str("addr"), if (a.has("thumb")) a.bool("thumb") else null, a.str("mode", "auto")
        )
    }
    ...
    private val readHexdump = EngineToolHandler(
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "editSessionId" str "Edit session ID (optional)"
                    "locator" str
                        "Section locator. Accepts full locator from analyze_elf sections, so_section:.text, or short section name like .text."
        ...
        e.hexdump(
            ...
            a.str("editSessionId"),
            a.str("locator"),
            a.intValue("byteOffset"),
            a.intValue("maxBytes", 4096)
        )
    }
    ...
    private val editHex = EngineToolHandler(
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "editSessionId" str "Edit session ID"
                    "locator" str
                        "Section locator for offset-based patching. Accepts full locator from analyze_elf sections, so_section:.text, or short section name like .text."
        ...
        if (vaStr.isNotEmpty()) {
            val va = HexCodec.long(vaStr)
                ?: return@EngineToolHandler err(
                    "INVALID_ARGUMENT",
                    "va must be a hexadecimal address",
                    "va",
                    vaStr
                )
            val patch = HexCodec.bytes(a.str("patchHex"))
                ?: return@EngineToolHandler err(
                    "INVALID_HEX",
                    "patchHex must contain valid byte pairs",
                    "patchHex",
                    a.str("patchHex")
                )
        ...
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "editSessionId" str "Edit session ID"
                    "locator" str
                        "Function or address locator. Accepts so_function:file!name@0xVA, function name, or bare 0xVA."
                    "dryRun" bool "If true, return a preview without applying changes"
        ...
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "editSessionId" str "Edit session ID"
                    "locator" str
                        "Symbol locator for rename. Accepts full locator from analyze_elf symbols or short symbol name."
                    ...
                    "name" str
                        "Symbol name (add: new function name, remove: symbol to remove, shortcut)"
                    "op".oneOf(
                        "rename | add | remove (shortcut, bypasses edits[])",
                        "rename",
                        "add",
                        "remove"
                    )
        ...
        if (op.isNotEmpty()) {
            when (op) {
                "add" -> HexCodec.long(a.str("addr"))?.let {
                    ...
                    ?: err(
                        "INVALID_ARGUMENT",
                        "addr must be a hexadecimal address",
                        "addr",
                        a.str("addr")
                    )

                "remove" -> e.liefRemoveSymbol(
                    a.str("workspaceId"),
                    a.str("editSessionId"),
                    a.str("name")
                )
        ...
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "symbolName" str
                        "Exported symbol to call, e.g. JNI_OnLoad or Java_com_example_Class_method. Use analyze_elf dynsyms first."
                    "args" arr
                        "Array of integer/string arguments after implicit JNI args for Java_* methods"
                    "trace" bool
                        "Enable verbose Unidbg tracing for diagnostics; use only on small functions"
                }
            )
        }
    ) { e, a, s ->
        if (!s.emulationEnabled) {
            err(
                "EMULATION_DISABLED",
                "Emulation is disabled in settings. Enable emulationEnabled to use this feature.",
                "emulationEnabled",
                false
            )
        } else {
    ...
    private val emulateDump = EngineToolHandler(
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "editSessionId" str "Edit session ID (optional)"
                    "addr" str
                        "Unidbg runtime absolute virtual address. Add the module base from unidbg_session(action=modules) to an ELF RVA/VA."
        ...
        if (!s.emulationEnabled) {
            ...
            val addr = a.str("addr").trim().removePrefix("0x").removePrefix("0X").toLongOrNull(16)
                ?: return@EngineToolHandler err(
                    "INVALID_ARGUMENT",
                    "addr must be a hex Unidbg runtime absolute virtual address",
                    "addr",
                    a.str("addr")
                )
            e.dumpMemory(
    ...
    private val rizinApi = EngineToolHandler(
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "command" str
                        "Any Rizin core command for action=command. Mutating/file/shell/debugger commands require unsafe=true."
                    "unsafe" bool
                        "Allow mutating, file, shell, debugger, and external commands. Requires authenticated MCP access."
        ...
        when (a.str("action", "analyze")) {
            ...
            "asm" -> e.assembleRaw(
                ...
                a.str("editSessionId"),
                a.str("asm"),
                HexCodec.long(a.str("addr")) ?: 0L,
                if (a.has("thumb")) a.bool("thumb") else null,
                a.str("mode", "auto")
            )
            ...
            "disasm" -> e.disasm(
                ...
                if (a.has("thumb")) a.bool("thumb") else null, a.str("mode", "auto")
            )

            "decompile" -> e.rzDecompile(
                a.str("workspaceId"),
                a.str("editSessionId"),
                a.str("locator").ifBlank {
                    a.str("addr")
    ...
    private val liefApi = EngineToolHandler(
        ...
        when (a.str("action", "parse")) {
            "capabilities" -> ok(
                e.capabilityRegistry().getJSONObject("backends").getJSONObject("lief")
            )

            "dispatch" -> e.liefDispatch(
                a.str("workspaceId"),
                a.str("editSessionId"),
                a.str("op", "roots"),
                a.str("objectPath"),
            ...
            "parse_any" -> e.liefDispatch(
                a.str("workspaceId"),
                a.str("editSessionId"),
                "parse_any",
                args = JSONArray().put(a.str("format", "auto"))
            )
            ...
            "patch_address" -> e.liefPatchAddress(
                a.str("workspaceId"),
                a.str("editSessionId"),
                HexCodec.long(a.str("va"))
                    ?: return@EngineToolHandler err(
                        "INVALID_ARGUMENT",
                        "va must be hex",
                        "va",
                ...
                HexCodec.bytes(a.str("patchHex"))
                    ?: return@EngineToolHandler err(
                        "INVALID_HEX",
                        "patchHex must be valid hex",
                        "patchHex",
                        a.str("patchHex")
                    )
            )

            "add_export" -> e.liefAddExportedFunction(
                a.str("workspaceId"),
                a.str("editSessionId"),
                HexCodec.long(a.str("va"))
                    ?: return@EngineToolHandler err(
            ...
            "remove_symbol" -> e.liefRemoveSymbol(
                a.str("workspaceId"),
                a.str("editSessionId"),
                a.str("name")
            )
            ...
            "build" -> e.build(
                ...
                a.str("editSessionId"),
                a.str("outputName"),
                a.str("conflictStrategy"),
                a.optBoolean("writeReport", s.writePatchReport),
                a.optBoolean("writeToWorkDir", s.buildCopyToWorkDir)
            )
    ...
    private val unidbgApi = EngineToolHandler(
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "args" arr
                        "For native_tool: [emulatorSessionId, toolName, toolArgumentsObject]. Other dispatch operations use their documented positional arguments."
                    "dispatchArgs" arr
                        "Alias for args when action=dispatch; kept for schema compatibility"
                }
            )
        }
    ) { e, a, s ->
        when (a.str("action", "status")) {
            "capabilities" -> ok(
                e.capabilityRegistry().getJSONObject("backends").getJSONObject("unidbg")
            )

            "dispatch" -> e.unidbgDispatch(
                a.str("workspaceId"),
                a.str("editSessionId"),
                a.str("op", "status"),
                a.str("method"),
                a.optJSONArray("args") ?: a.optJSONArray("dispatchArgs") ?: JSONArray()
            )

            "status" -> ok(e.emulationStatus().put("enabled", s.emulationEnabled))

            "call" -> if (!s.emulationEnabled) {
                err("EMULATION_DISABLED", "Emulation is disabled", "emulationEnabled", false)
            } else {
                e.emulate(
                    a.str("workspaceId"),
                    a.str("editSessionId"),
            ...
            "dump" -> if (!s.emulationEnabled) {
                err("EMULATION_DISABLED", "Emulation is disabled", "emulationEnabled", false)
            } else {
                e.dumpMemory(
                    a.str("workspaceId"),
                    a.str("editSessionId"),
                    HexCodec.long(a.str("addr"))
                        ?: return@EngineToolHandler err(
                            "INVALID_ARGUMENT",
                            "addr must be hex",
                            "addr",
    ...
    private val unidbgSession = EngineToolHandler(
        ...
        val dispatchArgs = when (a.str("action", "open")) {
            ...
            "call" -> JSONArray().put(
                sessionId
            ).put(a.str("symbolName")).put(callArgs).put(a.bool("trace", false))
            ...
            else -> return@EngineToolHandler err(
                "UNKNOWN_ACTION",
                "Unknown Unidbg session action",
                "action",
                a.str("action")
            )
        }
    ...
    private val unidbgMemory = EngineToolHandler(
        ...
        val dispatchArgs = when (a.str("action", "maps")) {
            "map" -> JSONArray().put(
                sessionId
            ).put(a.str("addr")).put(a.intValue("size", 4096)).put(a.intValue("prot", 3))
            ...
            "protect" -> JSONArray().put(
                sessionId
            ).put(a.str("addr")).put(a.intValue("size", 4096)).put(a.intValue("prot", 1))
    ...
    private val unidbgDebug = EngineToolHandler(
        ...
        val op = when (action) {
            ...
            "debugger_plan", "trace_plan", "breakpoints_plan" -> action
                ...
                "UNKNOWN_ACTION",
                "Unknown Unidbg debug action",
                "action",
                action
            )
        }
        val dispatchArgs = when (action) {
            "trace_code" -> JSONArray().put(
                a.str("emulatorSessionId")
            ).put(a.str("begin")).put(a.str("end"))

            "trace_start" -> JSONArray().put(
                a.str("emulatorSessionId")
            ).put(a.str("traceType", "code")).put(a.str("begin")).put(a.str("end"))

            "trace_events" -> JSONArray().put(
                a.str("emulatorSessionId")
            ).put(a.intValue("cursor", 0)).put(a.intValue("limit", 100))

            "trace_stop" -> JSONArray().put(a.str("emulatorSessionId")).put(a.str("traceId"))

            "trace_clear" -> JSONArray().put(a.str("emulatorSessionId"))

            "hook_start" -> JSONArray().put(
                a.str("emulatorSessionId")
            ).put(a.str("hookType", "syscall")).put(a.str("begin")).put(a.str("end"))
        ...
        ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "steps" arr
                        "Array of steps: {op, method?, args?, resultKey?}. Placeholders like ${'$'}{open.emulatorSessionId} are supported."
    ...
    private val xansoApi = EngineToolHandler(
        ...
        when (a.str("action", "status")) {
            "capabilities" -> ok(
                e.capabilityRegistry().getJSONObject("backends").getJSONObject("xanso")
            )

            "dispatch" -> e.xansoDispatch(
                a.str("workspaceId"),
                a.str("editSessionId"),
                a.str("op", "status")
            )

            "status", "help" -> e.xansoDispatch(
                a.str("workspaceId"),
                a.str("editSessionId"),
                a.str("action")
            )

            "build-section", "fix_sections" -> e.xansoBuildSections(
                a.str("workspaceId"),
                a.str("editSessionId"),
                a.bool("force", false)
            )
    ...
    private val sessionHistory = object : ToolHandler {
        override val meta = ToolMeta(
            ...
            objectSchema(
                props {
                    "action".oneOf(
                        "snapshot (default) | rollback | undo | redo | reset | check",
                        "snapshot",
                        "rollback",
                        "undo",
                        "redo",
        ...
        override fun handle(ctx: ToolContext, args: JSONObject): JSONObject {
            ...
            return when (args.str("action", "snapshot")) {
                "snapshot" -> e.editSnapshot(
                    args.str("workspaceId"),
                    args.str("editSessionId"),
                    args.str("label")
                )

                "rollback" -> e.editRollback(
                    args.str("workspaceId"),
                    args.str("editSessionId"),
                    args.intValue("snapshotIndex", -1)
                )

                "undo" -> e.editUndo(
                    args.str("workspaceId"),
                    args.str("editSessionId"),
                    args.intValue("count", 1)
                )
                ...
                "check" -> e.editCheck(args.str("workspaceId"), args.str("editSessionId"))
                    ...
                    "UNKNOWN_ACTION",
                    "Unknown action: ${args.str("action")}",
                    "action",
                    args.str("action")
                )
            }
    ...
    private val sessionAudit = object : ToolHandler {
        override val meta = ToolMeta(
            ...
            objectSchema(
                props {
                    "action".oneOf(
                        "audit (default) | persist | list | load",
                        "audit",
                        "persist",
                        "list",
                        "load"
        ...
        override val meta = ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "outputs" arr
                        ("Array of output variants (multi-build)" to SchemaBuilder.outputsSchema())
                    "conflictStrategy".oneOf(
                        "skip | overwrite | rename (default)",
                        "skip",
                        "overwrite",
                        "rename"
                    )
                ...
                "list" -> e.listBuildOutputs(args.str("prefix"), args.intValue("limit", 200))
                    ...
                    if (outputs != null && outputs.length() > 0) {
                        e.buildMany(
                            ...
                            args.str("editSessionId"),
                            outputs,
                            args.str("conflictStrategy"),
                            args.optBoolean("writeReport", s.writePatchReport),
                            args.optBoolean("writeToWorkDir", s.buildCopyToWorkDir)
                        )
                    } else {
                        e.build(
                            args.str("workspaceId"),
    ...
    private val systemControl = object : ToolHandler {
        override val meta = ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "publicUrl" str
                        "Named tunnel public HTTPS hostname/URL to display and persist (tunnel_start)"
        ...
        override fun handle(ctx: ToolContext, args: JSONObject): JSONObject {
            val hooked =
                ctx as? HookedContext
                    ?: return JSONObject().put("error", "System hooks not available")
            return when (args.str("action", "status")) {
                "status" -> hooked.sysStatusHook(args.bool("probe", false))

                "tunnel_start" -> hooked.tunnelStartHook(
                    args.str("mode", "quick"),
                    args.intValue("targetPort", 0),
                    "",
                    args.optString("publicUrl").takeIf {
                        it.isNotBlank()
                    }
                )
    ...
    private val appConfig = object : ToolHandler {
        override val meta = ToolMeta(
            ...
            objectSchema(
                props {
                    "action".oneOf(
                        "get (default) | set | schema | reset_token",
                        "get",
                        "set",
                        "schema",
                        "reset_token"
        ...
        override fun handle(ctx: ToolContext, args: JSONObject): JSONObject {
            ...
            return when (args.str("action", "get")) {
                ...
                "reset_token" -> ok(
                    ...
                    ).put("authEnabled", settings.authEnabled)
                )
                ...
                "set" -> {
                    val patch = when {
                        ...
                        args.optString("config").isNotBlank() -> runCatching {
                            JSONObject(args.optString("config"))
                        }.getOrElse {
                            return err(
                                "INVALID_CONFIG",
                                "config must be a JSON object: ${it.message}"
                            )
                        }
                    ...
                    settings.applyPatch(
                        patch,
                        allowSecrets = args.bool("allowSecrets", true),
                        allowSecurityFields = false
                    )
                }
    ...
    private val metaInfo = object : ToolHandler {
        override val meta = ToolMeta(
            ...
            objectSchema(
                props {
                    ...
                    "transactional" bool
                        "If true, snapshot edit sessions before batch steps and rollback them when a later step fails"
        ...
        override fun handle(ctx: ToolContext, args: JSONObject): JSONObject {
            val hooked =
                ctx as? HookedContext
                    ?: return JSONObject().put("error", "Meta hooks not available")
